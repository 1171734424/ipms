...
$("input[name='gdsdown']").bind("click",function() {
   ...
   function goodsup(e) {
	     var saletype = "singlesale";
         var gdsid = $(e).parent().parent().children("td").eq(1).html();
         var gdsname = $(e).parent().parent().children("td").eq(2).html();
         var gdsprice = $(e).parent().parent().children("td").eq(4).html();
         $.ajax({
				url : base_path + "/gdsApartUp.do",
				type : "post",
				dataType : "json",
				data : {
					'gdsid' : gdsid
