...
function setcleanrestAmount() {
	...
	$.ajax({
		url : path + "/setApartmentamount.do",
		dataType : "json",
		type : "post",
		data : {
			'time' : time,
...
function setcleanrestAmount2() {
	...
	$.ajax({
		url : path + "/setApartmentamountinit.do",
		dataType : "json",
		type : "post",
		data : {
...
// 处理保洁申请
function dealCleanApply(btn,contractId,status) {
...
if(status == 1){
	...
	$.ajax({
		url : base_path + "/queryApartmentRest.do",
		dataType : "json",
		type : "post",
		data : {
		...
		if(json.result == 1){
			...
			if (amount > 0) {
				JDialog.open(
								"安排保洁人员",
								base_path
										+ "/page/apartment/apartmentLeftmenu/apartmenthandleapply/arrangeClean.jsp?cleanApplication="
										+ cleanApplicationId
										+ "&getCleanTime="
	...
	$.ajax({
		url : base_path + "/cancleApartcleanapply.do",
		dataType : "json",
		type : "post",
		data : {
			'cleanApplicationId' : cleanApplicationId,
...
function showDetail(e){
	...
	window.parent.JDialog.open("保洁申请详情",base_path+"/showApartcleanapplydetail.do?cleanApplicationId="+cleanApplicationId
