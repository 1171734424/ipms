...
function addRepairRecord(){
	...
	$.ajax({
        url: path + "/addRepairApartApplicationRecord.do",
		 type: "post",
		 data : {"roomId":roomid,
				 "person":person,
				 "mobile":mobile,
		 ...
		 success: function(json) {
					 if(json.result =="1" ||json.result=="4" || json.result=="6"){
						 ...
						 $(window.parent.document.getElementById("repairIframe")).attr("src","repairApartmentData.do"); 
					 }else{
						 showMsg(json.message);
					 }
		 },
        ...
        url: path + "/addRepairApplicationRecordH.do",
		 ...
		 success: function(json) {
					 if(json.result =="1" ||json.result=="4" || json.result=="6"){
						 showMsg(json.message);
						 setTimeout(" window.parent.JDialog.close();",1500);
						 $(window.parent.document.getElementById("repairIframe")).attr("src","queryApartRepairDetail.do?startdate="+startdate+"&enddate="+" "+"&roomid="+" "+"&equipment="+" "+"&checkstatus="+" "+"&mobile="+" "); 
					 }else{
						 showMsg(json.message);
					 }
		 },
...
function queryRepairDetail(){
	...
	$("#repairIframe").attr("src","queryApartRepairDetail.do?startdate="+startdate+"&enddate="+enddate+"&roomid="+roomid+"&equipment="+equipment+"&checkstatus="+checkstatus+"&mobile="+mobile);
}
...
function selectRoomIdDetail(){
	...
	//获取公寓户型
	$.ajax({
		url: path+"/queryApartmentRoomType.do",
		type: "post",
		data: {},
		success: function(json){
			var li ="";
...
//添加房间id确定按钮单击事件
 ...
 function showDetail(e){
	...
	window.parent.JDialog.open("维修申请记录详情",base_path+"/dblApartRepairDetail.do?applyId="+applyId,950,300);
 }
 ...
 function cancelApplication(e){
	...
	var mainstatus = $($(e).parent().parent().find(".status")).html();
	 $.ajax({
         url: path + "/cancelApartmentRepair.do",
		 type: "post",
		 data : {"applyId" : applyId,"mainstatus" : mainstatus },
		 success: function(json) {
				$(window.parent.parent.document.getElementById("contentFrame_first")).click();
 ...
 function doneApplication(e){
	 ...
	 if((applystatus == "已审核" && post == "*") || applystatus == "未修复"){
		 window.parent.JDialog.open("添加修复信息", base_path+"/doneApartApplication.do?applyId="+applyId, 700,400);
	 }else{
		 if(applystatus == "已审核"){
 ...
 function addRepairLog(){
	 ...
	 $.ajax({
		 url: path + "/addApartRepairLogRecord.do",
		 type: "post",
	 ...
	 success: function(json) {
		 if(json.result=="4"){
			 ...
			 setTimeout(" window.parent.JDialog.close();",1500);
			 if(json.data == true){
				 $(window.parent.document.getElementById("repairIframe")).attr("src","repairApartInHouseData.do");
			 }else{
...
function showLog(e){
	 var applyId = $($(e).parent().parent().find("td").get(1)).html();
	 window.parent.JDialog.open("维修日志详情",base_path+"/loadApartmentRepairLog.do?applyId="+applyId,1200,300);
...
$(document).on("click","#repairPerson",function(){
	JDialog.open("维修人员 ", path +"/selectApartRepairPerson.do",450,350);
}); 
