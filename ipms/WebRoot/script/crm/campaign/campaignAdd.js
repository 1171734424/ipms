	...
	//调用checkFormInputLength
	 ...
	 if(!checkFormIsNull(arr) ){
	     return false;
		  ...
		  return false;
	 }else if(!checkTimeIsRight("startTime", "endTime", null, "开始时间", "结束时间", null)){
		  return false;
	 }else{
		 if($("#businessId").val()=="1"){
			...
			$.ajax({
				...
				success : function(json){
					if(json.result == 1){
						...
						window.parent.showMsg("该时段已有相似活动,是否覆盖?",continueAdd);
					}else{
...
function loadPageByBusinessId(){
	if($("#businessId").val() == "1"){
		...
		usingPersonnotselected();
		var innerDetail ="<table id='campaign1' class='specialone' name='campaign1'><tr><td class='state'><label>归属店</label></td><td><input id='branchId' name='branchId' value='' /><input id='branchId_CUSTOM_VALUE' name='branchId_CUSTOM_VALUE' type='hidden' value='' /></td><td class='state' style='width:22%;'><label id='labelhousechange'>特惠房型</label></td>"+
		 "<td><input id='roomType' name='roomType' value='' /><input id='roomType_CUSTOM_VALUE' name='roomType_CUSTOM_VALUE' type='hidden' value='' /></td><td class='state' style='width:22%;'>"+
		 "<label>特惠价格</label></td><td><input id='discountPrice' name='discountPrice' value='' /></td><td><label style='margin-left:115px;'>元</label></td><tr>"+
		...
		var innerDetail ="<table id='campaign1' class='specialone' name='campaign1'><tr><td class='state'><label id='belongTab'>归属门店</label></td><td><input id='branchId'"+
			" name='branchId' value='' /><input id='branchId_CUSTOM_VALUE' name='branchId_CUSTOM_VALUE' type='hidden' value='' /></td><td class='state' style='width:22%;'><label>特惠房间</label></td>"+
			"<td><input id='roomId' name='roomId' value='' /><input id='roomId_CUSTOM_VALUE' name='roomType_CUSTOM_VALUE' type='hidden' value='' /></td><td class='state' style='width:22%;'>"+
			"<label>特惠价格</label></td><td><input id='discountPrice' name='discountPrice' value='' /></td><td><label style='margin-left:115px;'>元</label></td><tr>"+
			"</tr><tr><td><label>显示始时</label></td><td><input id='startShowTime' name='startShowTime' value='' /></td><td><label>显示末时</label></td><td><input id='endShowTime' name='endShowTime' value='' /></td></tr></table>";
		$("#campaignDetail").html(innerDetail);
		$("table.specialone").css("margin-left","10px");
		$("label.yuan").css("position","relative");
		...
		"<td colspan='2'><input id='roomCount' name='roomCount' value='' /><label  style='margin-left:12px;font-size:12px;color:#333;'>天</label></td>"+
		"<td class='state' colspan='2'></td></tr>"+
		"<tr><td class='state' colspan='2'><label>送</label></td>"+
		"<td colspan='2'><input id='experienceCount' name='experienceCount' value='' type='text' /><label style='margin-left:12px;font-size:12px;color:#333;'>天</label></td>"+
		"</tr>"+
		"</table>";
		$("#campaignDetail").html(innerDetail);
		usingPersonselected();
	}else if($("#businessId").val() == "12"){
		var innerDetail = "<table id='campaign1' name='campaign1'>"+
		"<tr><td class='state' colspan='2' width='14%'><label>使用积分</label></td>"+
		"<td colspan='2'><input id='operScore' name='operScore' value='' /><label  style='margin-left:12px;font-size:12px;color:#333;'>分</label></td>"+
		"<td class='state' colspan='2'></td></tr>"+
		...
		"<td colspan='2'><input id='operMoney' name='operMoney' value='' /><label  style='margin-left:12px;font-size:12px;color:#333;'>元</label></td>"+
		"<td class='state' colspan='2'></td></tr>"+
		"</table>";
		$("#campaignDetail").html(innerDetail);
		usingPersonselected();
	}else if($("#businessId").val() == "14"){
		var innerDetail = "<table id='campaign1' name='campaign1'>"+
		"<tr><td class='state' colspan='2' width='14%'><label>使用积分</label></td>"+
		"<td colspan='2'><input id='operScore' name='operScore' value='' /><label  style='margin-left:12px;font-size:12px;color:#333;'>分</label></td>"+
		"<td class='state' colspan='2'></td></tr>"+
...
function continueAdd(){
	$.ajax({
		url:base_path + '/campaignsAdd.do?checkflag="nocheck"',
		type:"post",
		data : $("#basicInfo").serialize(),
		success : function(json){
			if(json.result == 1){
