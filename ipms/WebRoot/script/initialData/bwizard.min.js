		...
		f.fn[b] = function(e) {
			...
			if (n) {
				this.each(function() {
							...
							if (!k) {
								return f.error("cannot call methods on " + b
					...
					"bootstrap.bwizard",
					{
							...
							if (this.list) {
								...
								this.lis.each(function(j) {
											if (e.clickableSteps) {
												...
												a(this).contents().wrap(
																'<a href="#step' + (j + 1) + '" class="hidden-phone"/>');
											} else {
												a(this).contents().wrap(
						...
						_refreshStep : function() {
							...
							if (this.buttons && !c.loop) {
								...
								this.nextBtn[c.activeIndex >= this.panels.length - 1 ? "addClass"
										: "removeClass"]
										("disabled")
										.attr(
												"aria-disabled",
												(c.activeIndex >= this.panels.length - 1));
							}
						},
						...
						remove : function(c) {
							...
							if (d.hasClass("bwizard-activated")
								...
								this.show(c+ (c < this.panels.length ? 0
														: -1));
							}
