					...
					function() {
						...
						if (MemberId) {
							JDialog.open("", base_path
									+ "/hotelscoreExchange.do?MemberId=" + MemberId
...
function monitorinput(){
	$("#userorder").on("input propertychange",function(){
		$("#user").val($(this).val());
	})
	
	$("#ordemobile").on("input propertychange",function(){
		$("#mobile").val($(this).val());
	})
	
	$("#ordergendor").on("input propertychange",function(){
		$("#gender").val($(this).val());
	})
	
//	$("#ordergendor").change(function(){
//		alert("变化了");
//	})
...
function selectmember() {
	...
	if (Mnumber) {
		$.ajax({
			url : base_path + "/hotelmemberSelect.do",
			type : "post",
			dataType : "json",
			data : {
				'Mnumber' : Mnumber
			...
			success : function(json) {
				if (1 == json.result) {
					if (json.message) {
						...
						JDialog.open("", base_path + "/hoteljudgeUser.do?Mnumber="
								+ Mnumber+"&arrivedate="+arrivaltime, 300, 150);
					}
					$("#newMemberId").val(json.data);
					readonlyinput();
				} else {
...
function readonlyinput(){
	$("#userorder").attr("disabled", true);
	$("#ordergendor").attr("disabled", true);
	$("#ordemobile").attr("disabled", true);
}
...
$("#roomid").bind(
		...
		function() {
			...
			if (roomtype) {
				JDialog.open("", base_path + "/hotelroomSelect.do?roomtype="
...
/* 新选择房型刷对应房价 */
function newrtChange() {
	if(webstatus=="inline"){
		$("#roomid").val("");
		var t = $("#roomtype").val();
		$.ajax({
			url : base_path + "/hotelorderprice.do",
			type : "post",
			dataType : "json",
			data : {
				roomType: t,
				memberId: $("#newMemberId").val()
			},
			success : function(json) {
				if(json.result == 1){
					var res = json.data;
					$("#price").val(res.roomprice);
					$("#rpid").val(res.rpid);
					$("#limited").val(res.limit);
				}
			},
			error : function() {
			}
...
function orderdata() {
	...
	$.ajax({
		url : base_path + "/hotelorderRoom.do",
		type : "post",
		dataType : "json",
		data : {
		...
		success : function(json) {
			if (1 == json.result) {
				if (json.message) {
					...
					showMsg("预订成功!");
					window.setTimeout("window.parent.parent.parent.location.reload(true)",1800);
				}
