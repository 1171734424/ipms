...
//	})
	...
	//判断是否断网
	$.ajax({//依据会员级别刷房价
		url: path+"/leftmenucheckConnection.do",
		type: "post",
		data: {
	},
	success: function(json){
...
$(document).on("click","#roomid",function(){
	...
	if(roomtype == 0){
		...
		$.ajax({//依据房型查房间号
			url: path+"/leftmenuqueryRoomId.do",
			type: "post",
			data: {"roomtype" : roomtype},
			success: function(json){
				if(json.result =="1"){
...
//	var OrdeUser = $("#idnumber").val();//入住人身份证
	...
	if(hourRoomFlag){//钟点房+hours小时
		...
		if(formatDate(new Date())>= checkoutTime){
			showMsg("预离时间不能选择当天!");
			return;
		}
	}
	...
	if(noConnectionFlag){
		...
		//断网开始入住
		$.ajax({
			url: path+"/leftmenuCheckInDirectWithNoConnection.do",
			type: "post",
		...
		//联网入住开始
		$.ajax({
			url: path+"/leftmenuCheckInDirectWithConnection.do",
			type: "post",
			data: {"checkUser" : checkUser,
			"roomType" : roomType,
			"roomId" : roomId,
		...
		success: function(json){
			...
			if(json.result=="2"){
				...
				//if($(window.parent.parent.document.getElementById("tolive")).hasClass("active")){
					...
					window.setTimeout("window.parent.parent.parent.location.reload(true)",1500);
					setTimeout(" window.parent.parent.JDialog.close();",1800);
				//}
...
//	}
	...
	if(hourRoomFlag == true){
			showMsg("确认切换为普通房吗?",function(){
				...
				$.ajax({
					...
					//url: path+"/leftmenushowRoomPriceDirectLive.do",
					type: "post",
					data: {
					"roomtype" : roomtype,
	...
	}else { //if(!$("#checkouttime").prop("disabled"))
...
//	limitFlag = false;//限制钟点房点击按钮
	...
	$.ajax({
		url: path+"/leftmenugetGuestInfo.do",
		type: "post",
		data: {
		"roomtype" : roomtype,
		"idCard" : idCard,
