...
function addRepairRecordH(){
	...
	$.ajax({
        url: path + "/addRepairApplicationRecordHouse.do",
		 type: "post",
		 ...
		 success: function(json) {
					 if(json.result =="1" ||json.result=="4" || json.result=="6"){
						 ...
						 $(window.parent.document.getElementById("repairIframe")).attr("src","repairInHouseDataInHouse.do"); 
					 }else{
						 showMsg(json.message);
					 }
		 },
...
function queryRepairDetail(){
	...
	$("#repairIframe").attr("src","queryRepairDetailInHouse.do?startdate="+startdate+"&enddate="+enddate+"&roomid="+roomid+"&equipment="+equipment+"&checkstatus="+checkstatus+"&mobile="+mobile);
}
...
function selectRoomIdDetailH(){
	...
	//获取公寓户型
	$.ajax({
		url: path+"/queryRoomTypeInHouse.do",
		type: "post",
		data: {},
		success: function(json){
			var li ="";
...
document.documentElement.onclick=function(e){
 ...
 function showSimpleRoom(e){
	 ...
	 $.ajax({
         url: path + "/showSimpleRoomInHouse.do",
		 type: "post",
...
//添加房间id确定按钮单击事件
 function chooseRoom(){
 		...
 		$.ajax({//自动获取
 			url: path + "/getApplicationDateInHouse.do",
 			type: "post",
 ...
 function showDetail(e){
	...
	window.parent.JDialog.open("维修申请记录详情",base_path+"/dblRepairDetailInHouse.do?applyId="+applyId,950,300);
 }
 ...
 function doneApplication(e){
	 ...
	 if((applystatus == "已审核" && post == "*") || applystatus == "未修复"){
		 window.parent.JDialog.open("添加修复信息", base_path+"/doneApplicationInHouse.do?applyId="+applyId, 700,390);
	 }else{
		 if(applystatus == "已审核"){
 ...
 function addRepairLog(){
	 ...
	 $.ajax({
		 url: path + "/addRepairLogRecordInHouse.do",
		 type: "post",
...
function showLog(e){
	 var applyId = $($(e).parent().parent().find("td").get(1)).html();
	 window.parent.JDialog.open("维修日志详情",base_path+"/loadRepairLogInHouse.do?applyId="+applyId,1200,300);
...
$(document).on("click","#repairPerson",function(){
	...
	JDialog.open("维修人员 ", path +"/selectRepairPersonInHouse.do",450,350);
}); 
