...
function selectmember() {
	...
	if (Mnumber) {
		$.ajax({
			url : base_path + "/memberSelectInHouse.do",
			type : "post",
			dataType : "json",
			data : {
				'Mnumber' : Mnumber
...
$("#addnewtab")
		.click(function() {
			...
			var imgid = "img_"+ InputCount;
				$("#tabtitlenew").append(
						' <li class="tabli active" id=' + tabtitle
				...
				$("#tabCon")
						.append(
								'<div id='
										+ tabCon
										+ ' style="display:none" class="hiddenuser"> <ul id="tabcontent" class="clearfix magin_label_top">'
										...
										+'<li><label class="orlabel_name orlabel_red">身份证</label><input id= '+idcard+' name="check_idcard" type="text"  class="check order_input"  maxlength="18" ></li>'
										+'</ul><input type="hidden" class="OrdermemberId" name="OrdermemberId"></div>');
				...
				$(".tabli").last().siblings().removeClass("active");
			});
