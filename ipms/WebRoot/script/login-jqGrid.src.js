...
$.extend($.jgrid,{
	...
	getCellIndex : function (cell) {
		var c = $(cell);
		if (c.is('tr') || c.is('span') || c.is('li')) { return -1; }
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		.bind('click', function(e) {
			...
			ci = $.jgrid.getCellIndex(td);
			$(ts).triggerHandler("jqGridRightClickRow", [$(ptr).attr("id"),ri,ci,e]);
			if ($.isFunction(ts.p.onLeftClickRow)) { ts.p.onLeftClickRow.call(ts,$(ptr).attr("id"),ri,ci, e); }
		});
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 19)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			case "textarea" :
				...
				if(!options.rows) { options.rows = 4; }
...
$.jgrid.extend({
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				var nm, hc,trdata, cnt=0, /*br = 0,*/tmp, dc,elc, retpos=[], ind=false, 
				...
				$(obj.p.colModel).each( function(i) {
					...
					if(this.editrules && this.editrules.edithidden === true) {
						...
						hc = this.edithidden === true ? true : false;
					}
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						//cp = parseInt((parseInt(frmopt.colpos,10) || 2)*2,10);
						...
						if (rowid != "_empty" && this.readonly) {
							$(elc).attr("disabled", true);
						}
						//if (br % 2 == 0) {
						...
						if (trdata.length===0 /*&& br % 2 == 0) || obj.p.colModel[i].edittype == "textarea"*/) {
							trdata = $("<tr "+dc+" rowpos='"+rp+"'></tr>").addClass("FormData").attr("id","tr_"+nm);
							$(trdata).append(tmpl);
							$(tb).append(trdata);
							trdata[0].rp = rp;
						}
						$("td:eq("+(cp /*- (br + 1) % 2 * 2*/ - 2)+")",trdata[0]).html(frmopt.label === undefined ? obj.p.colNames[i]: frmopt.label);
						$("td:eq("+(cp /*- (br + 1) % 2 * 2*/ - 1)+")",trdata[0]).append(frmopt.elmprefix).append(elc).append(frmopt.elmsuffix);
						
						if (obj.p.colModel[i].editrules && obj.p.colModel[i].editrules.required) {
							$("td:eq("+(cp /*- (br + 1) % 2 * 2*/ - 2)+")",trdata[0]).css("color", "#f00");
						}
						//if (obj.p.colModel[i].edittype == "textarea") {
						//	br++;
						//}
			...
			bS  ="<div id='sData' style='float:right;background-color:#f7f7f7;background: #4A5064;color: #fff;margin-top: 4px;' class='dialogButton' title='ALT+ENTER'>"+p.bSubmit+"</div>",
			bC  ="<div id='cData' style='margin-right:30px;float:right;margin-top: 5px;' class='dialogButton' title='ESC'>"+p.bCancel+"</div>";
			...
			$("#"+$.jgrid.jqID(IDs.themodal)).keydown( function( e ) {
				...
				if(rp_ge[$t.p.id].savekey[0] === true && e.which === rp_ge[$t.p.id].savekey[1] && e.altKey) { // save
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			bN = "<a id='"+bn+"' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>",
			bC  ="<div id='cData' style='padding:0;margin-right:30px;float:right;margin-top: 7px;' class='dialogButton'>"+p.bClose+"</div>";
			if(maxRows >  0) {
				var sd=[];
				$.each($(tbl)[0].rows,function(i,r){
					sd[i] = r;
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode >= 37 && e.keyCode <= 40) {
						...
						var srow = iRow, scol = iCol;
						if (e.keyCode === 38 && iRow > 1) {
							srow--;
							$($t).jqGrid("nextCell", iRow - 1, iCol - 1);
						} else if (e.keyCode === 40 && iRow < $t.rows.length) {
							srow++;
							$($t).jqGrid("nextCell", iRow + 1, iCol - 1);
						} else if (e.keyCode === 37 && iCol > 1) {
							scol--;
						} else if (e.keyCode === 39 && iRow < $t.p.colModel.length) {
							scol++;
						}
						
						try {
							selectCell($t.rows[srow].id, scol);
						} catch(e) {
						}
					...
					if (e.keyCode === 13) {
						...
						if (cm.customCallback) {
							callCustomFunc(cm.customCallback, iRow, e.keyCode);
						}
						
						try {
							selectCell($t.rows[iRow].id, iCol);
						} catch(e) {
						}
	...
	GridNav : function() {
		return this.each(function () {
			...
			.keydown(function (e){
				...
				try {
					selectCell($t.rows[$t.p.iRow].id, scol);
				} catch(e) {
				}
				return false;
			});
