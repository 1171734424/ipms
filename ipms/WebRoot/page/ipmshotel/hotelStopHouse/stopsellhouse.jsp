   	...
   	<div class="header_roomlist">
		...
		<section class="box-content-section fl"  style="width:100%;">
			<section class="box_content_widget fl">
				...
				<iframe name="logFrame" id="logFrame" frameborder="0" width="100%" height="100%" src="hotelShowstophouse.do?status=1&type=${param.type}"></iframe>
				</div> 
			</section>
		</section>
		</div>
		...
		<script type="text/javascript">
			...
			function getQuerydata(status){
				...
				if(typeof(status)!= "undefined"){
					$("#status").val(status);
					$("#logFrame").attr('src',path + "/hotelShowstophouse.do?status=" + status + "&type=" + type);				
				}
				else{
					$("#logFrame").attr('src',path + "/hotelShowstophouse.do?" + $("#halt_search").serialize());	
				}
			}
			...
			$(".createhouse").on("click",function(){
				window.parent.JDialog.open("增加停售房", path + "/page/ipmshotel/hotelStopHouse/stophouse_add.jsp",832,462);
