  ...
  <body>
				...
				<tbody id="info">
					<c:forEach items="${list}" var="ordinfo" varStatus="status">
						<tr>
							...
							<c:choose>
								<c:when test="${ordinfo['SOURCE'] ne '3' && ordinfo['STATUS'] eq '1' }">
									<td><input type="button" class="checkroom" value="退款" onclick="aptOrderReFund('${ordinfo['ORDER_ID'] }','${ordinfo['STATUS'] }')"/></td>
								</c:when>
								<c:when test="${ordinfo['SOURCE'] eq '3' && ordinfo['STATUS'] eq '6' }">
									<td><input type="button" class="checkroom" value="退款" onclick="aptOrderReFundTwo('${ordinfo['ORDER_ID'] }','${ordinfo['STATUS'] }')"/></td>
								</c:when>
								
								
								<c:otherwise>
									<td><input type="button" class="new_checkroom" value="退款" /></td>
								</c:otherwise>
							</c:choose>
						</tr>
					</c:forEach>
	...
	<script>
		...
		function aptOrderReFund(aptOrderId,status){
	 	...
	 	function aptOrderReFundTwo(aptOrderId,status){
			if(status == 6){
				$.ajax({
			         url: path + "/aptOrderReFundTwo.do",
					 type: "post",
					 data : {aptOrderId : aptOrderId},
					 success: function(json) {
						 showMsg(json.message);
