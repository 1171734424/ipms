  ...
  <body>
   	<div class="bill_width">
		...
		var workbillid = '<%=request.getParameter("workbillid")%>';
			$(function(){
				$.ajax({
			         url: path + "/apartmentloadWorkBillData.do",
					 type: "post",
					 data : {
				...
				$.ajax({
			         url: path + "/apartmentloadpayandcostworkbill.do",
					 type: "post",
			...
			function showaddbill(){
				...
				window.parent.JDialog.open("入账", path + "/apartmentshowgetAddWorkBill.do?workbillid=" + workbillid , 700, 400);
			}
			...
			function consumption(){
				...
				$.ajax({
			         url: path + "/apartmentcheckAllRights.do",
					 type: "post",
					 data : {
					 ...
					 success: function(json) {
					 	if (json.result == 1) {
					 		JDialog.open("", path + "/page/apartment/apartmentmainmenu/apartmentworkbill/consumptionworkbill.jsp?strdetailid=" + strdetailid,680,360);
					 	} else {
						 	showMsg(json.message);
						}
					 },
			...
			function checkout(){
				...
				$.ajax({
			         url: path + "/apartmentcheckoutworkbill.do",
					 type: "post",
					 data : {workbillid : workbillid},
					 success: function(json) {
						 if(json.result == 1){
				...
				$.ajax({//查找账单的备注信息
					 ...
					 success: function(json) {
					 	if(json.result == "2"){
							//获取当前td的内容
							text = json.message;
							window.parent.JDialog.open("修改备注",path+"/apartmentreWriteWKRemark.do?text="+text+"&billId="+billId,450,200);
					 	}
					 },
					 error: function(json) {}
				});
			...
			function addRemark(e){
				...
				var remark = $(e).val();
				$.ajax({
			         url: path + "/apartmentaddRemarkToBill.do",
					 type: "post",
					 data : {billId : billId,
					 		remark : remark
					 },
