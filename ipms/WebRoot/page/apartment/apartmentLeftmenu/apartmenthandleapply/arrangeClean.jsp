  ...
  <input type="hidden" id="memberid" name="memberid" value="<%=request.getParameter("memberid")%>"/>
        <section class="detail_one">
            <ul class="clearfix">
                ...
                <li><label class="label_name">保洁人：</label><input id="repairPerson" name="repairPerson" class="text_search" onclick="getHire()" readonly>
                <input type="hidden" id="repairPerson_CUSTOM_VALUE" name="repairPerson_CUSTOM_VALUE">
...
<!--		       <input type="button" id="submitbill" value="取消" onclick="window.parent.JDialog.close()" class="button_margin cancel"/>-->
    ...
    function getallstaff(){
  		$.ajax({
		         url: base_path + "/selectApartallstaff.do",
				 type: "post",
				 data : {},
  	...
  	function getHire(){
  		window.JDialog.open("保洁人员", base_path + "/selectRepairPersonInHouse.do", 450, 350);
  	}
    ...
    function updatecleanrecord(e){
			...
			$.ajax({
		         url: base_path + "/updateApartcleanrecord.do",
				 type: "post",
				 dataType:"json",
