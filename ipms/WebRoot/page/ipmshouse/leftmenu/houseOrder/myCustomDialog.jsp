	...
	<body oncontextmenu="return true;">
		...
		<script><!--
			...
			function chooseHouse(){
				var houseId = window.parent.$("#houseName_CUSTOM_VALUE").val();
				var houseName = window.parent.$("#houseName").val();
				var startTime = window.parent.$("#arrivedate").val();
				var endTime = window.parent.$("#leavedate").val();
				$.ajax({
			        url: base_path + "/houseChoose.do",
					 ...
					 success: function(json) {
							if(json.result =="0" || json.result == "2"){
								window.parent.$("#houseName").val("");
								window.parent.$("#houseName_CUSTOM_VALUE").val("");
								window.parent.$("#price").val("");
								showMsg(json.message);
								
								return;
							}
							if(json.result =="1"){//民宿可订
								window.parent.$("#houseName").val(houseName);
								window.parent.$("#price").val(json.message);
								window.parent.JDialog.close();
							}
					 },
					 error: function(json) {
