...
<html>
  ...
  <style>
	...
	.js-max-input {border: solid 1px #bfbcbc;background: white;padding: 0 10px 0 10px;font-size:20px;color: black}
  </style>
  </head>
  
  <body>
    <div class="bill_width">
			<div class="bill_one" style="height:415px;">
			...
			<div class="bill_three">
				...
				<span class="">帐面金额<span class="money" id="accountfee">0.00</span></span>
<%--				<span class="">平账金额<span class="money" id="totalfee">1.00</span></span>--%>
			</div>
			<div class="foot_submit" id="foot_submit" style="margin-top: 20px;">
				<span class="button_margin" onclick="showaddbill()" id="showaddbill">入账</span>
				<span class="button_margin" onclick="consumption()">冲减</span>
 				<span id="houseRefuntd" class="button_margin" onclick="checkout()" style="display:none;">结账</span>
		...
		<script type="text/javascript" src="<%=request.getContextPath()%>/script/ipmsinhouse/workbillroomInHouse/Zoom.js"></script>
		<script src="<%=request.getContextPath()%>/script/common/keyPrevent.js"></script>
		<script type="text/javascript">
		var path = "<%=request.getContextPath() %>";
		var checkid = '<%=request.getParameter("checkid")%>';
		var status = '<%=request.getParameter("status")%>';
		var guarantee = '<%=request.getParameter("guarantee")%>';
			$(function(){
				loadbill();
				loadpayandcost();
				//loadroommapping();
			...
			function resetprice(){
				...
				window.parent.JDialog.open("房费调整", path + "/page/ipmshouse/leftmenu/houserefund/resetroompriceinhouse.jsp?checkid=" + checkid , 420,180);
			}
			...
			function loadpayandcost(){
				$.ajax({
			         url: path + "/loadHousePayandCost.do",
					 type: "post",
			...
			function loadroommapping(){
				$.ajax({
			         url: path + "/houseloadroommapping.do",
					 type: "post",
					 data : {checkid : checkid},
					 success: function(json) {
			...
			function loadallbill(){
				$.ajax({
			         url: path + "/loadHouseBillDataInHouse.do",
					 type: "post",
					 data : {
					 	checkid : checkid,
					 	status : '1,2,3,4,0'
			...
			function showaddbill(){
				...
				window.JDialog.open("入账", path + "/showHouseAddBill.do?checkid=" + checkid , 500, 300);
			}
			...
			function loadroombilldata(json){
				var tabledata,desc;
			 	$.each(json, function(index){
				 		if(status == '1'){
			 			tabledata = tabledata + "<tr class='odd' " + (json[index]["STATUS"] == 0?"style='color: #999'":"") + ">" +
				 		...
				 		"<td class='desc'>" + json[index]["DESCRIBE"] + "</td>" +
				 		/* "<td>" + json[index]["STAFFNAME"] + "</td>" + */
				 		"<td class='date' style='white-space:nowrap'>" + dealDate(json[index]["RECORDTIME"]) + "</td>" +
				 		"<td>" + json[index]["PAYMENT"] + "</td>"+
				 		"<td class='refund' style='display:none'>" + json[index]["REFUNDSTATUS"] + "</td>"+
				 		"<td onclick='ReWriteTd(this)' class='parentCls'> " + converttosomething(json[index]["REMARK"], null, '')  + "</td>" +
				 		"</tr>";
			...
			function consumption(){
					...
					if(arr[index]["checked"]){
						if($(arr[index]["parentNode"]["parentNode"]["childNodes"][1]).find("span").length != 1){
							...
							if("保洁费" == desc){
								...
								//if( date.split(" ")[0] < dealLocalDate(new Date()).split(" ")[0]){
									showMsg("日结不可冲减!");
									flag = false;
									return;
								//}
				...
				JDialog.open("", path + "/page/ipmshouse/leftmenu/houserefund/consumption.jsp?strbillid=" + strbillid +"&checkid=" + checkid,680,360);
			}
			...
			function checkout(){
				...
				$.ajax({
			         url: path + "/houseOrderoff.do",
					 type: "post",
					 data : {checkid : checkid},
					 success: function(json) {
					 	if(json.result == 1){
							JDialog.open("退房", path + "/page/ipmshouse/leftmenu/houserefund/checkout.jsp?checkid=" + checkid, 350, 160);
						}else {
							showMsg(json.message);
						}
					 },
			...
			function ReWriteTd(e){
				...
				var billId = $(e).siblings().eq(1).html();
				$.ajax({//查看该账单对应的房单的状态
					 ...
					 success: function(json) {
					 	...
					 	if(json.result == "2"){
							//获取当前td的内容
							text = json.message;
							window.JDialog.open("修改备注",path+"/reWriteRemarkH.do?text="+text+"&billId="+billId,450,200);
					 	}
					 },
					 error: function(json) {}
				});
