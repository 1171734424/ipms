   	...
   	<div class="detail_wrap_margin detail_color">
			...
			<div class="order_margin house_margin">
				<section class="detail_one">
					...
					<ul class="clearfix">
						<li><label class="label_name">民宿编号</label><input name="民宿编号" type="text" id="BRANCH_ID" class="check room_select" disabled></li>
						...
						<li id="adressshow" style='display:none;'>
							<label class="label_name">默认地址</label>
							<input id="adressshowcontent"  type="text" class="checkAdressMail" disabled>
						</li>
			...
			<section class="detail_four" >
				<div class="fr" role="button">
					<ul class="clearfix">
						...
						<li id="lookHouseOrder" onclick="lookHouseOrder();"><span class="button_margin button_color">查看房单</span></li>
						<li id="checkout" onclick="popuphouse()"><span id="changehouse_span" >换房</span></li>
						<li id="checkout" onclick="overstay()"><span id="overstay_span" >续住</span></li>
						<li id="checkin" onclick="adjustprice();"><span id="adjustprice_span">单价调整</span></li>
					</ul>
				</div>
			</section>
			</div> 
		...
		<script>
			...
			function overstay(){
				if(checkDo == 1){
					window.JDialog.open("续住", path + "/page/ipmshouse/housecheck/overstay.jsp?leavetime=" + $("#leavetime").val() + "&houseorderid=" + houseorderid, 610, 330);
				}
			}
				...
				$.ajax({
					 ...
					 success: function(json) {
					 	...
					 	if(json[0]["STATUS"] != '1'){
							...
							$("#changehouse_span").addClass("check_span");
							$("#overstay_span").addClass("check_span");
							$("#adjustprice_span").addClass("check_span");
							...
							initcheckusercss("2", "0");
					 		...
					 		$("#changehouse_span").addClass("button_margin");
							$("#overstay_span").addClass("button_margin");
							$("#adjustprice_span").addClass("button_margin");
			...
			function quertHouseOrderAddress(){
				$.ajax({
			         url: path + "/quertHouseaddress.do",
					 type: "post",
					 dataType: "json",
					 data : {houseCheckId : houseorderid},
					 success: function(json) {
						 if(json != null && json.length != 0){
						 	$("#adressshow").css("display","block");
							//$("#recipients").text(json[0]["RECIPIENTS"]);
						 	//$("#mobile").text(json[0]["MOBILE"]);
					 		//$("#mailcode").text(json[0]["MAILCODE"]);
						 	//$("#address").text(json[0]["ADDRESS"]);
						 	$("#adressshowcontent").val(json[0]["RECIPIENTS"]+"/"+json[0]["MOBILE"]+"/"+json[0]["MAILCODE"]+"/"+json[0]["ADDRESS"]);
						 } else {
						 	$("#adressshow").css("display","none");
						 }
					 },
					 error: function(json) {}
				});
			...
			function adjustprice(){
				if(checkDo == 1){
					window.JDialog.open("价格调整", base_path + "/adjustorderprice.do?orderid=" + houseorderid, 460, 240);
				}				
			...
			function checkOut() {
				...
				$.ajax({
			         url: path + "/isCheckOutDate.do",
					 type: "post",
					 dataType: "json",
					 data : { housecheckId: houseorderid },
					 success: function(json) {
					 	if (json.result == 0) {
					 		showMsg("是否确定退房?",function(){
					 			loading();
					 			$.ajax({
									 ...
									 success: function(json) {
									 	...
									 	if (json.result == 0) { 
									 		showMsg(json.message);
									 		window.setTimeout(window.parent.document.getElementById('menu_904').click(),'1500');
									 		window.setTimeout(window.parent.JDialog.close(),'1500');
									 	} else if (json.result == 1) {
									 		showMsg(json.message);
									 	}	
									 },
					 		...
					 		window.JDialog.open("民宿退房", path + "/page/ipmshouse/housecheck/secondquery.jsp?orderId=" + houseorderid , 300,370);
		...
		function lookHouseOrder(){                            
			window.parent.JDialog.openWithNoTitle("", path + "/page/ipmshouse/housecheck/houseOrder_check.jsp?houseorderid=" + houseorderid , 1179, 733);
		}
	</script>
  </body>
</html>
