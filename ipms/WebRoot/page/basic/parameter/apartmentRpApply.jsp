						   ...
						   <tr>
							   ...
							   <td align="center" class="rpapply_table_td">${roomType }</td>
	                              <td align="center" class="rpapply_table_td">
							   		<div class="apply_table_edit_input" style="float: left;">
								    ...
								    <select id="priceType" name="priceType">
								    	<c:if test="${ theme == 1 }">
							         	<option value="1" <c:if test="${ priceType == 1}">selected="selected"</c:if>>日租</option>
							         	<option value="2" <c:if test="${ priceType == 2}">selected="selected"</c:if>>时租</option>
								    	</c:if>
							         	<c:if test="${ theme == 2 }">
							         	<option value="3" <c:if test="${ priceType == 3}">selected="selected"</c:if>>长租</option>
							         	</c:if>
								   	</select>
							    </td>
							</tr>
...
<!-- 		<script src="<%=request.getContextPath()%>/script/common/tipInfo.js"></script> -->
	    ...
	    $(document).ready(function(){
			...
			$.ajax({
				url: base_path+"/apartQueryApartdjustRules.do",
				type: "post",
				dataType: "json",
				data: {},
