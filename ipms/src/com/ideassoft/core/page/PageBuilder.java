...
public class PageBuilder extends GenericDAOImpl implements IPageBuilder{
	...
	public Map<String, Object> queryData(String modelId, String pageId, Map<String, Object> params, 
		...
		if (pageConfig.getStrategy() != null) {
			...
			} else if(SystemConstants.SqlStrategy.MULTISQL.equals(pageConfig.getStrategy())){
				sqlOrProc = findMultiSQL(sqlOrProc, true);
			}else {
	...
	public List<?> queryCustomDialogData(String dialogTarget, String dialogColumns, 
		...
		for (int i = 0; i < targets.length; i++) {
			...
			if (pagination != null) {
				...
				pagination.setRecords(records);
				//考虑整除的总页码
				if(records % pagination.getRows() == 0){
					pagination.setTotal(records / pagination.getRows());
					}else{
	...
	public Object findByCond(String beanName, String colName, String colValue, 
		...
		String hql;
		Object bean = ReflectUtil.getBeanByName(beanName);
		String status = ReflectUtil.getTurelyFieldName(bean, "status");
		if (status != null) {
			hql = " from " + beanName + " where " + colName + " = '" + colValue + "'" + cond + " and status in(1,2,3)";
		} else {
			hql = " from " + beanName + " where " + colName + " = '" + colValue + "'" + cond;
		}
