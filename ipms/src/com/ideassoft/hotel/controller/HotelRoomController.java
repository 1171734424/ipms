...
public class HotelRoomController {
	...
	public ModelAndView showresetprice(HttpServletRequest request, String checkid) {
		...
		ModelAndView mv = new ModelAndView();
		mv.addObject("checkid", checkid);
		mv.addObject("roomprice", check.getRoomPrice());
...
//	}
	...
	@RightMethodControl(rightType = RightConstants.RightType.HOT_COBI)
...
//	}
	...
	public void submitgetcontinuebill(HttpServletRequest request, HttpServletResponse response, String checkId,
		...
		Bill bill = new Bill();
		Check check = (Check) roomService.findOneByProperties(Check.class, "checkId", checkId);
		Order order = (Order) roomService.findById(Order.class, checkId);
		...
		try {
			...
			RoomUtil.addsaveCheckinPrice(DateUtil.s2d(cuurent_checkouttime, "yyyy/MM/dd hh:mm"), DateUtil.addDays(DateUtil.s2d(getcontinuetime, "yyyy/MM/dd hh:mm"), -1), 
					check.getRoomType(), "MSJ", staff.getBranchId(), "1", checkId, staff.getStaffId(), null, order.getOrderUser());
	...
	public void changeRoom(HttpServletRequest request, HttpServletResponse response, String roomid, String checkid) {
		...
		Room room = (Room) roomService.findOneByProperties(Room.class, "roomKey.branchId", branchid, "roomKey.roomId", roomid);
		SinglePrice sin = new SinglePrice(branchid, "1", room.getRoomType(), "MSJ", RealPrice.PriceType.DATE, new Date(), order.getOrderUser());
		/*
		...
		String lastbranchId = lastcheck.getBranchId();
		String lastroomType = lastcheck.getRoomType();
		Branch br = (Branch)roomService.findOneByProperties(Branch.class, "branchId", lastbranchId);
		if(br != null){
			lastbranchname = br.getBranchName();
		}
		...
		switchroom.setCurrrpId(lastcheck.getRpId());
		switchroom.setCurrRoomid(roomid);
		switchroom.setCurrRoomtype(room.getRoomType());
		switchroom.setCurrRoomprice(sin.checkRoomPrice());
		...
		try {
			...
			roomService.updatecheck(lastcheck.getRpId(), roomid, room.getRoomType(), sin.checkRoomPrice(), checkid);
			RoomUtil.cancelRoommap(lastroomid,branchid);
			RoomUtil.updateCheckinPricenew(new Date(), DateUtil.addDays(lastcheck.getCheckoutTime(), -1), room.getRoomType(), "MSJ", branchid, "1", checkid, null, order.getOrderUser());
			//酒店换房
			...
			List<?> checkUserList = roomService.findByProperties(CheckUser.class, "checkId", order.getOrderId(), "checkinType", "2");
			if(checkUserList != null && checkUserList.size() > 0){
				for (int j = 0; j < checkUserList.size(); j++){
					...
					if(idcardUser != null && !StringUtil.isEmpty(idcardUser)){
						try {
							Equipment equipment = (Equipment) roomService.findOneByProperties(Equipment.class, "branchId", lastbranchId, "roomId", lastroomid);
							if(equipment != null){
							...
							if(result != CommonConstants.PortalResultCode.FAILED && !result.toString().equals(CommonConstants.doorInterfaceResult.FAILED)){
								...
								if(equipment != null){
									...
									warningLog.setRemark(lastbranchname+"["+lastroomType+"/"+lastroomid +"]"+"删除锁中所有身份证信息中["+idcardUser+"]失败");
			...
			operlog.setLogId(DateUtil.currentDate("yyMMdd") + order.getBranchId() + roomService.getSequence("SEQ_OPERATELOG_ID"));
...
//	}
	...
	public void loadCustomerData(HttpServletRequest request, HttpServletResponse response, String checkid,
		...
		RoomMapping roommapping = (RoomMapping) roomService.findOneByProperties(RoomMapping.class, "relaRoomid", roomid, "branchId",branchId);
		List<?> listroommapping = roomService.findByProperties(RoomMapping.class, "roomId", roommapping.getRoomId(), "branchId",branchId);
...
/*	@SuppressWarnings("unchecked")
	...
	public void loadtransferorderData(HttpServletRequest request, HttpServletResponse response, String checkid) {
		...
		List<?> result = roomService.findBySQL(sql, new String[] { branchId, order.getOrderUser(), order.getSource(),check.getCheckId(), branchId }, true);
		JSONUtil.responseJSON(response, result);
	}
...
//	}
	...
	public void autoautotransferbill(HttpServletRequest request, HttpServletResponse response, String checkid) {
		...
		RoomMapping roommapping = (RoomMapping) roomService.findOneByProperties(RoomMapping.class, 
				"relaRoomid", hostroomid, "branchId",branchId);
		if (roommapping == null) {
		...
		for (RoomMapping roomMapping2 : listroommapping11) {
			Check roommapcheck = (Check) roomService.findOneByProperties(Check.class, 
					"roomId", roomMapping2.getRelaRoomid(), "status", CommonConstants.CheckStatus.CheckOn, "branchId", branchId);
...
//			}
	...
	public ModelAndView forwardRoom(HttpServletRequest request, HttpServletResponse response, String madate,
		...
		Date date = null;
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
		List<?> rt = roomService.findByProperties(RoomType.class, "theme", "1", "roomTypeKey.branchId", branchId,"status","1");

		//循环遍历查出的房型，将房型一一提取出来，到数据库中在查数量
		for (int x = 0; x < rt.size(); x++) {
			...
			for (int y = 0; y < day; y++) {
				...
				list = roomService.findBySQLWithPagination("queryforward", new String[] { branchId, branchId, branchId, newdate,
						 newdate, branchId, newdate, newdate, branchId, branchId, newdate, newdate, branchId }, pagination, true);
	...
	public void checkoutbill(HttpServletRequest request, HttpServletResponse response, CheckoutRoom checkoutroom) {
		...
		try{
			...
			//TODO
			//查询check的状态，如果是3,即已经点击过退房，若不是则需要删除门锁里的人
			//酒店一个单子结算,先判断check的状态，如果已经是已退未结，就不需要再删门锁里的客人了
			String message = "";
			String checkStatus = check.getStatus();
			if(checkStatus != null && !StringUtil.isEmpty(checkStatus) && !checkStatus.equals("3")){
				List<?> checkUserList = roomService.findByProperties(CheckUser.class, "checkId", check.getCheckId(), "checkinType", "2");
				if(checkUserList != null && checkUserList.size() > 0){
					for (int j = 0; j < checkUserList.size(); j++){
						String idcardUser = ((CheckUser)(checkUserList.get(j))).getIdcard();
						checkUserLis += idcardUser +",";
						if(idcardUser != null && !StringUtil.isEmpty(idcardUser)){
							try {
								Equipment equipment = (Equipment) roomService.findOneByProperties(Equipment.class, "branchId", check.getBranchId(), "roomId", check.getRoomId());
								if(equipment != null){
									result = CardUtil.doorDelCardData(request, response, equipment.getSerialNo(), idcardUser);
								}
								...
								if(result != CommonConstants.PortalResultCode.FAILED && !result.toString().equals(CommonConstants.doorInterfaceResult.FAILED)){
									...
									if(equipment != null){
										...
										warningLog.setStatus(CommonConstants.servieLogStatus.EFFECTIVE);
										warningLog.setWarningType(CommonConstants.warningLogStatus.DEL_ALL_CARD_FAILED);
										warningLog.setRemark("["+check.getBranchId()+"/"+check.getRoomId()+"]删除锁中所有身份证信息中["+idcardUser+"]失败");
										warningLog.setSerialNo(equipment.getSerialNo());
										roomService.save(warningLog);
									}
								}	 
			...
			RoomUtil.cancelRoommap(roomid,branchId);
			
			OperateLog operlog = new OperateLog();
			operlog.setBranchId(check.getBranchId());
			operlog.setLogId(DateUtil.currentDate("yyMMdd") + check.getBranchId() + roomService.getSequence("SEQ_OPERATELOG_ID"));
			String operid = InetAddress.getLocalHost().toString();// IP地址
			operid = (String) operid.subSequence(operid.indexOf("/") + 1, operid.length());
			operlog.setOperIp(operid);
			operlog.setOperType("11");
			operlog.setOperModule("结算操作(酒店)");
			operlog.setRecordUser(loginuser.getStaff().getStaffId());
			operlog.setRecordTime(new Date());
			
			if(failFlagCount == 0){
				if(checkUserLis != ""){
					message = "房单号" + check.getCheckId() + "已结账,入住人["+checkUserLis.substring(0, checkUserLis.length()-1)+"]门锁里身份证信息删除成功";
					operlog.setContent(message);		
				} else {
					message = "房单号" + check.getCheckId() + "结算成功，入住人退房时已删";
					operlog.setContent(message);				
				}
				roomService.save(operlog);
				...
				if(checkUserLis != ""){
					message = "房单号" + check.getCheckId() + "已结账,入住人中["+checkUserLis.substring(0, checkUserLis.length()-1)+"]门锁里有["+checkUserLisFail.substring(0, checkUserLisFail.length()-1)+"]身份证信息删除失败";
					operlog.setContent(message);
				} else {
					message = "房单号" + check.getCheckId() + "结账算功,入住人退房时已删";
					operlog.setContent(message);				
				}
				roomService.save(operlog);
				JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, "结算成功，门锁里身份证信息删除失败!"));
			}
		}
		catch(Exception e){
			e.printStackTrace();
	...
	public void allcheckoutbill(HttpServletRequest request, HttpServletResponse response, CheckoutRoom checkoutroom) {
		...
		List<?> listroommapping1 = roomService.findByProperties(RoomMapping.class, "roomId", roommapping.getRoomId(), "branchId",branchId);
		...
		try{
			...
			for (Object object : listroommapping1) {
				...
				//酒店集体结算
				//点击过退房的，个人结算的不在里面
				//酒店结算，变离店状态，删除原房间的入住人 
				//门锁里删除住户身份证信息,而不删除保洁的身份证信息
				checkListarray += relacheckid +",";
				List<?> checkUserList = roomService.findByProperties(CheckUser.class, "checkId",relacheckid, "checkinType", "2");
				if(checkUserList != null && checkUserList.size() > 0){
					for (int j = 0; j < checkUserList.size(); j++){
						String idcardUser = ((CheckUser)(checkUserList.get(j))).getIdcard();
						checkUserLis += idcardUser +",";
						if(idcardUser != null && !StringUtil.isEmpty(idcardUser)){
							try {
								Equipment equipment = (Equipment) roomService.findOneByProperties(Equipment.class, "branchId", check.getBranchId(), "roomId", roomid);
								if(equipment != null){
									result = CardUtil.doorDelCardData(request, response, equipment.getSerialNo(), idcardUser);
								}
			...
			operlog.setOperIp(operid);
			operlog.setOperType("11");
			operlog.setOperModule("集体结算操作(酒店)");
			operlog.setRecordUser(loginuser.getStaff().getStaffId());
			operlog.setRecordTime(new Date());
			
			if(failFlagCount == 0){
				operlog.setContent("房单号" + checkListarray.substring(0, checkListarray.length()-1)+"]门锁已结算,入住人["+checkUserLis.substring(0, checkUserLis.length()-1)+"]门锁里身份证信息删除成功");
				roomService.save(operlog);
				JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, "集体结算成功!"));
			} else {
				operlog.setContent("房单号" + checkListarray.substring(0, checkListarray.length()-1)+"]门锁已结算,入住人中["+checkUserLis.substring(0, checkUserLis.length()-1)+"]门锁里有["+checkUserLisFail.substring(0, checkUserLisFail.length()-1)+"]身份证信息删除失败");
				roomService.save(operlog);
				JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, "集体结算成功，门锁里身份证信息删除失败!"));
			}
		}
		catch(Exception e){
			e.printStackTrace();
...
//	}
	...
	public void committransferorder(HttpServletRequest request, HttpServletResponse response, String checkid,
		...
		try {
			for (Object object : listbill) {
				Bill bill = (Bill)object;
				if(bill.getCost() == 0 && bill.getPay() == 0){
					continue;
				}
			...
			RoomUtil.addsaveCheckinPrice(order.getArrivalTime(), DateUtil.addDays(order.getLeaveTime(), -1), check.getRoomType(), 
					check.getRpId(), branchId, "1", checkid, staff.getStaffId(), null, order.getOrderUser());
	...
	public void getproject(HttpServletRequest request, HttpServletResponse response) {
		...
		String sqlallproejct = "queryallhotelproject";
	...
	public void setroommap(HttpServletRequest request, HttpServletResponse response, String strcheckid,
		...
		try {
			for (String checkid : arrcheck) {
				Check check = (Check) roomService.findOneByProperties(Check.class, "checkId", checkid);
				String roomid = check.getRoomId();
				RoomMapping roommapping = (RoomMapping) roomService.findOneByProperties(RoomMapping.class, "relaRoomid", hostroomid, "branchId",staff.getBranchId());
				if(roommapping == null){
					...
					roommapp.setBranchId(staff.getBranchId());
	...
	public void roomout(HttpServletRequest request, HttpServletResponse response, String checkid,String remark) {
		Integer failFlagCount = 0;
		String checkUserLis = "";
		String checkUserLisFail = "";
		Integer result = CommonConstants.PortalResultCode.FAILED;
		...
		try{
			...
			String cklogId = DateUtil.currentDate("yyMMdd") + branchId + roomService.getSequence("SEQ_CHECKOUTlOG_ID");
			cklog.setCheckoutId(cklogId);
			cklog.setBranchId(branchId);
			cklog.setCheckId(checkid);
			cklog.setCheckoutDate(new Date());
			cklog.setRecordUser(recordUser);
			cklog.setRemark(remark);
			cklog.setType("1");
			roomService.save(cklog);
			roomService.update(check);
			roomService.upateroomstatus(check.getBranchId(), roomId, CommonConstants.RoomStatus.RoomDirty);
			RoomUtil.cancelRoommap(roomId,branchId);
			//TODO 酒店退房
			//酒店退房，变已退未结，删除原房间的入住人 
			//门锁里删除住户身份证信息,而不删除保洁的身份证信息
			//先查询订单里的所有的身份证，不管状态
			List<?> checkUserList = roomService.findByProperties(CheckUser.class, "checkId", check.getCheckId(), "checkinType", "2");
			if(checkUserList != null && checkUserList.size() > 0){
			...
			if(failFlagCount == 0){
				...
				roomService.save(operlog);
				JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, "退房成功，门锁里身份证信息删除失败!"));
			}
		}
		catch(Exception e ){
			e.printStackTrace();
	...
	@RequestMapping("/ishaveroommapp.do")
	public void ishaveroommapp(HttpServletRequest request, HttpServletResponse response, String checkid){
		LoginUser loginuser = (LoginUser) request.getSession(true).getAttribute(RequestUtil.LOGIN_USER_SESSION_KEY);
		String branchId = loginuser.getStaff().getBranchId();
		Check check = (Check) roomService.findOneByProperties(Check.class, "checkId", checkid);
		RoomMapping roommapping = (RoomMapping) roomService.findOneByProperties(RoomMapping.class, "relaRoomid", check.getRoomId(), "branchId", branchId);
		if(roommapping == null){
			JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, "无关联房", 0));
		}else {
			JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, "存在关联房", 1));
		}
	}
	...
	public void loadGuestData(HttpServletRequest request, HttpServletResponse response, String checkuserid) throws JSONException {
		...
		if(StringUtils.isEmpty(checkuser.getRankName())){
			...
			if(!StringUtils.isEmpty(searchMessage)){
				member = portal.call(1, 1, "{ function: \"checkInDirect.getmemberhasRank\", data:{idcard:" +searchMessage  +" } }");
				if(!"null".equals(member)){
					JSONObject jsonmem = JSONObject.fromObject(member);
					checkuser.setRankName(jsonmem.getString("memberRank"));					
				}else{
					checkuser.setRankName(CommonConstants.MembenRank.NM);
				}
	...
	public void setCheckuserType(String checkId, String checkinType, CheckUser checkUser){
	...
	public void addcustomerHand(HttpServletRequest request, HttpServletResponse response, 
		...
		if(precheckuser != null){
			//判断是否有主客人，是否以主客人的身份住进去
			setCheckuserType(checkid, checkinType, precheckuser);
			...
			//判断是否有主客人
			setCheckuserType(checkid, checkinType, checkuser);
	...
	public void dlcheckuser(HttpServletRequest request, HttpServletResponse response, String checkuserId, 
		...
		CheckUser checkuser = (CheckUser) roomService.findOneByProperties(CheckUser.class, "checkuserId", checkuserId);
		//房单中的入住人删除时，清锁中的数据
		String checkid = checkuser.getCheckId();
		String idcardUser = checkuser.getIdcard();
		String checkinType = checkuser.getCheckinType();
		Check c = (Check)roomService.findOneByProperties(Check.class, "checkId", checkid);
		Contrart co = (Contrart)roomService.findOneByProperties(Contrart.class, "contrartId", checkid);
		Equipment et = null;
		String branchId = null;
		String checck = null;
		if(c != null && checkinType.equals("2")){
			checck = c.getCheckId();
			branchId = c.getBranchId();
			String roomId = c.getRoomId();
			if(branchId.contains("H")){
				et = (Equipment) roomService.findOneByProperties(Equipment.class, "branchId", branchId);			
			} else {
				et = (Equipment) roomService.findOneByProperties(Equipment.class, "branchId", branchId, "roomId", roomId);
			}
		} else if(co != null && checkinType.equals("2")){
			checck = co.getContrartId();
			branchId = co.getBranchId();
			et = (Equipment) roomService.findOneByProperties(Equipment.class, "branchId", co.getBranchId(), "roomId", co.getRoomId());
		}
		
		if(et != null){
			try {
				result = CardUtil.doorDelCardDataInTask(et.getSerialNo(), idcardUser);
				...
				if(result != CommonConstants.PortalResultCode.FAILED && !result.toString().equals(CommonConstants.doorInterfaceResult.FAILED)){
					...
					warningLog.setStatus(CommonConstants.servieLogStatus.EFFECTIVE);
					warningLog.setWarningType(CommonConstants.warningLogStatus.DEL_ALL_CARD_FAILED);
					warningLog.setRemark("单号["+checck+"]门店号["+branchId+"]删除锁中身份证["+idcardUser+"]删除失败");
					warningLog.setSerialNo(et.getSerialNo());
					roomService.save(warningLog);
		...
		try{
			...
			if(failFlagCount == 0){
				...
				JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, "删除客人成功,门锁内删除失败!", null));			
			}
		}catch(Exception e){
			e.printStackTrace();
			JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.FALSE, "删除客人失败!", null));			
	...
	public void searchmember(HttpServletRequest request, HttpServletResponse response,String checkid, String datamember, String checkinType) throws JSONException {
		...
		if(precheckuser == null){
			precheckuser = (CheckUser) roomService.findOneByProperties(CheckUser.class, "idcard", datamember, "checkId", checkid, "checkinType", checkinType);
		}
		if(precheckuser != null){
			List<?> listcheckuser = roomService.findByProperties(CheckUser.class, 
					"checkId", checkid, "checkinType", checkinType,
...
//	}
	...
	public void resetprice(HttpServletRequest request, HttpServletResponse response, String checkid, String amount, String remark) {
		...
		try {
			...
			roomService.save(check);
			RoomUtil.updateCheckinPricenew(new Date(), DateUtil.addDays(check.getCheckoutTime(), -1), check.getRoomType(), "MSJ", branchId, "1", checkid, "" + check.getRoomPrice(), order.getOrderUser());
	...
	public void veritfycheckout(HttpServletRequest request, HttpServletResponse response, String checkid) {
		...
		Check check = (Check) roomService.findOneByProperties(Check.class, "checkId", checkid);
		String roomid = check.getRoomId();
		RoomMapping roommapping = (RoomMapping) roomService.findOneByProperties(RoomMapping.class, "relaRoomid", roomid,"branchId",staff.getBranchId());
	...
	public ModelAndView showPromptRecordDetail(HttpServletRequest request, String logid, String checkid)
		...
		mv.addObject("pagination", pagination);
		mv.setViewName("page/ipmshotel/order/prompt_details");
		return mv;
	}
...
//	}
	...
	public void splitAccount(HttpServletRequest request,HttpServletResponse response, Bill billData, String numberone) throws IOException, ClassNotFoundException {
		...
		sb.append("&");
		String remark = "原账单为:"+String.format("%.2f", bill.getCost())+",拆分了"+numbers.length+"个,分别为：";
		sb.append(remark);
		for (int i=0;i<numbers.length;i++) {
			...
			sb.append(String.format("%.2f",Double.valueOf(numbers[i]))).append(",");
			roomService.save(newbill);
		}
		sb.delete(sb.length()-1,sb.length());
		bill.setStatus(CommonConstants.BillStatus.BillSplitAccount);
		if (bill.getRemark() != null) {
			bill.setRemark(bill.getRemark() + billData.getRemark()+ sb.toString());
		} else {
			bill.setRemark(billData.getRemark() + sb.toString());
		}
...
//	}
	 ...
	 * @Description 酒店房态图
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:21:05
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelstatus.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_STATUS)
	public ModelAndView hotelstatus(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
		mv.setViewName("page/ipmshotel/room_status/room_status");
		return mv;
	}
	/**
	 * 
	 * @Description 酒店房态统计
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:25:18
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelStatistics.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_STATIC)
	public ModelAndView hotelStatistics(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
		mv.setViewName("page/ipmshotel/room_statistics/room_statistics");
		return mv;
	}
	/**
	 * 
	 * @Description 酒店工作账
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:35:54
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelwork_account.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_ACCOUN)
	public ModelAndView hotelworkaccount(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
	 ...
	 * @Description 酒店报表
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:37:58
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelReportForms.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_RFORMS)
	public ModelAndView hotelReportForms(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
		mv.setViewName("page/ipmshotel/reportform/reportForms");
		return mv;
	}
	 ...
	 * @Description 酒店订单
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:40:55
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelOrder.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_ORDER)
	public ModelAndView hotelOrder(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
		mv.setViewName("page/ipmshotel/order/order");
		return mv;
	}
	 ...
	 * @Description 酒店房单
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:45:15
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelRoomList.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_CHECK)
	public ModelAndView hotelRoomList(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
		mv.setViewName("page/ipmshotel/room_statistics/roomlist");
		return mv;
	}
	 ...
	 * @Description 酒店日志
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:46:48
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelIntegral.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_INTEG)
	public ModelAndView hotelIntegral(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
		String type = request.getParameter("type");
		mv.setViewName("page/ipmshotel/hotelIntegral/integral");
		mv.addObject("type",type);
		return mv;
	}
	 ...
	 * @Description 酒店停售房
	 * @author ideas_mengl
	 * @date   2018年7月24日下午3:51:52
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping("/hotelStopHouse.do")
	@RightMethodControl(rightType = RightConstants.RightType.HOT_STOP)
	public ModelAndView hotelStopHouse(HttpServletRequest request, HttpServletResponse response){
		ModelAndView mv = new ModelAndView();
		String type = request.getParameter("type");
		mv.setViewName("page/ipmshotel/hotelStopHouse/stopsellhouse");
		mv.addObject("type",type);
		return mv;
	}
