...
import com.ideassoft.pmsinhouse.controller.HouseOrderController;
...
public class SellCtrl {
	...
	public void submithaltplanhouse(HttpServletRequest request, HttpServletResponse response, String roomId, String haltType,
		...
		if(ishaltplanoverlap(haltplan.getStartTime(), haltplan.getEndTime(), roomId, branchId)){
...
//					sellService.save(mlog);
			...
			String msg = "添加成功!";
			JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.SUCESS, msg));
		}
		catch(Exception e){
			e.printStackTrace();
		}
	...
	public void submithaltplanroom(HttpServletRequest request, HttpServletResponse response, String roomId, String haltType,
		...
		//如果添加的停售计划开始时间实当前时间,检查该房间是否已存在正在执行的停售计划
		if(!ishalplaning(haltplan.getStartTime(), roomId, branchId)){
			JSONUtil.responseJSON(response, new AjaxResult(CommonConstants.AJAXRESULT.FALSE, "已存在正在执行的停售计划,请先完成其他停售计划!"));
			return;
		}
...
//				sellService.save(mlog);
	...
	public boolean ishaltplanoverlap(Date startDate, Date endDate, String roomId, String branchId){
		List<?> listhaltplan = sellService.findBySQL("getvalidhaltplan", new String[]{roomId , branchId}, true);
		Map<?, ?> data;
		boolean flag = false;
	...
	/**
	 * 
	 * @Description 检测是否存在正在执行的停售计划
	 * @author ideas_mengl
	 * @date   2018年7月25日上午9:33:49
	 * @param startDate
	 * @param roomId
	 * @return
	 */
	public boolean ishalplaning(Date startDate, String roomId, String branchId){
		List<?> listhaltplanings = sellService.findBySQL("getvalidhaltplaning", new String[]{roomId, branchId}, true);
		if(DateUtils.isSameDay(startDate, new Date())){
			if(listhaltplanings.size() > 0){
				return false;
			}
			return true;
		}
