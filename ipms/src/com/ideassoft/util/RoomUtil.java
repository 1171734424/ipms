...
public class RoomUtil {
	...
	public static CheckAllAmount getCheckamount(HttpServletRequest request, String checkid, String strbillIds) {
		...
		SinglePrice sp = new SinglePrice(branchId, "1", check.getRoomType(), "MSJ", RealPrice.PriceType.HOUR, new Date(), order.getOrderUser());
		double hourprice = sp.checkRoomPrice();
		double subprice = 0;
	...
	/**
	 * 删除以前数据保存开始日期到结束日期的房租价，最后一个参数表示每日的价格，如果为null则会自动查价格表塞入
	...
	/**
	 * 追加保存开始日期到结束日期的房租价，最后一个参数表示每日的价格，如果为null则会自动查价格表塞入
	 * @param startDate 开始日期
	 * @param endDate 结束日期
	 * @param roomType 房型（适用于酒店、公寓） 民宿（null)
	 * @param rpId 房价码
	 ...
	 * @param recorduser 当前操作人
	 * @param amount 调整价格（可以 null，默认取价格表塞入）
	 * @param memberId 会员ID，自动录入价格会查询会员memberRank
	 */
	public static void addsaveCheckinPrice(Date startDate, Date endDate, String roomType, String rpId, String branchId,String theme, String orderId,String recorduser, String amount, String memberId) {
		Calendar start = Calendar.getInstance();
		start.setTime(startDate);
		int days = DateUtil.daysOfTwo(startDate, endDate);
		for(int i = 0; i <= days ; i++){
			...
			ordchePrice.setDataId(dataId);
			if(StringUtils.isEmpty(amount)){
				Double price = new SinglePrice(branchId, theme, roomType, rpId, "1", start.getTime(), memberId).checkRoomPrice();
				ordchePrice.setDayPrice(price);
			}else{
				ordchePrice.setDayPrice(Double.valueOf(amount));
			}
	 ...
	 * @param startDate 开始日期
	 * @param endDate 结束日期
	 * @param roomType 房型（适用于酒店、公寓） 民宿（null)， 用来查询价格表
	 * @param rpId 房价码 , 用来查询价格表
	 * @param branchId 门店
	 * @param theme 场景 1-酒店 2-公寓 3-民宿
	 * @param orderId 订单编号
	 * @param recorduser 当前操作人
	 ...
	 * @param recorduser 当前操作人
	 * @param amount 调整价格（可以 null，默认取价格表塞入）
	 * @param memberId 会员Id用来识别房价码折扣
	 */
	public static void updateCheckinPricenew(Date startDate, Date endDate, String roomType, String rpId, String branchId,String theme, String orderId, String amount, String memberId) {
		...
		for (Object object : list) {
			...
			Date date = (Date) map.get("WHICHDAY");
			if(StringUtils.isEmpty(amount)){
				Double price = new SinglePrice(branchId, theme, roomType, rpId, "1", date, memberId).checkRoomPrice();
				dayPrice = "" +  price;
			}else{
				dayPrice = amount;
			}
	...
	public static boolean cancelRoommap(String roomid,String branchId){
		RoomMapping roommapping = (RoomMapping) roomUtilService.findOneByProperties(RoomMapping.class, "relaRoomid", roomid, "branchId", branchId);
		if (roommapping == null) {
			return false;
		} else {
			List<Map<String, Object>> roommappinglist = roomUtilService.findallroommapping(roommapping.getRoomId(), branchId);
			try {
				if(roommappinglist.size() == 2){
					roomUtilService.deleteroommapping(roommapping.getRoomId(), branchId);
					return true;
				}else{
					roomUtilService.deleteroommappingbyroomid(roommapping.getRoomId(), roomid, branchId);
					return true;
				}
			} catch (Exception e) {
				e.printStackTrace();
...
/*	@SuppressWarnings("unchecked")
	...
	/**
	 * 传入非会员的手机号和身份证ID
	 * 如果查到会员信息手机号和身份证ID一致时，返回会员的memberId
	 * 如果身份证号或者手机号不一致时。需更新会员信息。
	 * 如果身份证和手机号都没有查到，则会生成新的会员记录，返回新的会员memberId
	 * @param response
	 * @param nonmobile
	 * @param nonidcard
	 * @return
	 ...
	 * 存在会返回memberId
	 * "2"表示是否填充会员信息
	 * null表示操作，非会员和会员信息不一致，需要跟新
	 */
	public static String checkMember(HttpServletResponse response, String nonmobile, String nonidcard ){
		...
		if(StringUtils.isEmpty(nonidcard))
			throw BusinessException.withErrorCode(ErrorConstants.Common.PARAM_IS_EMPTY);
		Member member = roomUtilService.findMemberhasRank(nonmobile, nonidcard);
		if(member != null){
			return member.getMemberId();
		}
		
		Member idmember = (Member) roomUtilService.findMemberhasRank(null, nonidcard);
		if(idmember != null){
			//身份证查明，该第三方非会员存在于会员之中，提取手机号是否存在，
			String mobile = idmember.getMobile();
			if(!StringUtils.isEmpty(mobile)){
				//存在手机号，手机号冲突，请去会员中心修改
				JSONUtil.responseJSON(response, new AjaxResult(0, "手机号冲突，请修改会员中心 !"));
				return null;
			}else{
			...
			//没有身份证，用手机号查询
			Member momember = (Member) roomUtilService.findMemberhasRank(nonmobile, null);
			if(momember != null){
//				String idcard = momember.getIdcard();
//				if(!StringUtils.isEmpty(idcard)){
					//存在身份证号，身份证号冲突，请去会员中心修改
					JSONUtil.responseJSON(response, new AjaxResult(0, "无身份证号，请完善会员信息 !"));
					return null;
				//}
				
			}else{
				//不存在身份证号，也没有手机号
				Member m = (Member) roomUtilService.findOneByProperties(Member.class, "mobile", nonmobile, "idcard", nonidcard);
				Member mem;
				try {
					mem = (Member) CloneUtil.deepClone(m);
					mem.setMemberId(roomUtilService.getSequence("SEQ_MEMBER_ID"));
					mem.setMemberRank(CommonConstants.MembenRank.PM);
					roomUtilService.save(mem);
					roomUtilService.newandsaveaccount(mem.getMemberId());
	 ...
	 * @return 返回memberId值
	 */
	public static String memupdate(String memberupdate, String memberId){
		
		Member mem = (Member) roomUtilService.findOneByProperties(Member.class, "memberId", memberId);
		
		if(!CommonConstants.MembenRank.NM.equals(mem.getMemberRank())){
			return mem.getMemberId();
		}
		
		String mobile = mem.getMobile();
		String idcard = mem.getIdcard();
			Member momember = (Member) roomUtilService.findMemberhasRank(mobile, null);
			if(momember != null){
				try {
					if("confirm".equals(memberupdate)){
//						Member newmem = (Member) CloneUtil.deepClone(roomUtilService.findOneByProperties(Member.class, "memberId", momember.getMemberId()));
//						newmem.setIdcard(idcard);
//						roomUtilService.update(newmem);
						roomUtilService.updateidcardbyid(momember.getMemberId(), idcard);
					}
					return momember.getMemberId();
				} catch (Exception e) {
					e.printStackTrace();
					return null;
				} 
			}
			Member idmember = (Member) roomUtilService.findMemberhasRank(null, idcard);
			if(idmember != null){
				try {
					if("confirm".equals(memberupdate)){
//						Member newmem = (Member) CloneUtil.deepClone(roomUtilService.findOneByProperties(Member.class, "memberId", idmember.getMemberId()));
//						newmem.setMobile(mobile);
//						roomUtilService.update(newmem);
						roomUtilService.updatemobilebyid(idmember.getMemberId(), mobile);
					}
					return idmember.getMemberId();
				} catch (Exception e) {
					e.printStackTrace();
					return null;
				} 		
	...
	public Member findMemberByIdCardMobile(String mobile, String idcard){
		Member member = roomUtilService.findMemberhasRank(mobile, idcard);
		if(member != null){
			return member;
		}
		return null;
	...
	public CheckUser findCheckUserByIdCardMobile(String checkid, String mobile, String idcard){
		CheckUser checkUser = (CheckUser) roomUtilService.findOneByProperties(CheckUser.class, "checkId", checkid, "mobile", mobile, "idcard", idcard);
		if(checkUser != null){
			return checkUser;
		}
		checkUser = (CheckUser) roomUtilService.findOneByProperties(CheckUser.class, "checkId", checkid, "mobile", mobile, "idcard", idcard);
		return null;
	}
}
