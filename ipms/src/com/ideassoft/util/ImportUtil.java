...
public class ImportUtil {
	...
	private void packageData(Object bean, ColumnConfig config, String cellValue, int pos, String checkMsg) throws Exception {
		...
		/*		if (isCellEmpty(cellValue))
				...
				//因为excel的时间传到这边的格式在setValue方法中没有，所以改成这样2018/08/03   zhongll
			...
			} else if ("dialog-radio".equals(config.getEditType())) {
				if (!"".equals(cellValue)) {
				...
				//由于之前没有dialog，但为了满足商品资料导入，添加了.逻辑和dialog-radio一样，但是添加了如果出现逗号则提示错误。2018/08/03   zhongll
				if (!"".equals(cellValue)) {
