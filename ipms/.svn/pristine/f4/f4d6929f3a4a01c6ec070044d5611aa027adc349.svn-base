...
function bindTabClick() {
	var movement = [ "leavedHydropowerLog","roomContractHydropower", "roomingHydropowerLog", "payHydropower"];
	$(".header_roomlist_li").on("click", function() {
		...
		if (movement[$(this).attr("index")] == "payHydropower") {
...
function change(select){
	...
	var timeHtml = "";
	if(select == 0){
		$("#condition").attr("action","leavedHydropowerLog.do");
		...
		$("#condition").attr("action","roomingHydropowerLog.do");
		html += "<li><label class='label_name'>合同号</label><input name='contractId' id='contractId' type='text' value='' id='id' class='text_search'></li>";
		html += "<li><label class='label_name'>房间号</label><input name='roomId' id='roomId' type='text' value='' id='id' class='text_search'></li>";
...
$("tr[class='odd']").dblclick(function(){
	var thisId = this.id;
	var typeTsf = "2";
	window.parent.JDialog.open("水电详情", base_path +"/roomingHydropowerLog.do?contractIdTsf="+thisId+"&typeTsf="+ typeTsf,1600,500);
})
...
function payHydropower() {
	...
	if (checked1.length > 0) {
		...
		window.parent.JDialog.open("水电批量支付", base_path + "/batchPayHydropower.do?myorderId="+myorderId+"&mymonth="+mymonth,750, 430);
