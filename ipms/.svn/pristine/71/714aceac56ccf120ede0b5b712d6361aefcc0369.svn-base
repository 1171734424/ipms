...
function querymyroomstatus() {
	$.ajax({
		url:base_path + "/queryApartmentRoomList.do",
		type:"post",
		dataType: "json",
				...
				if (json.state == 1) {
					...
					$(".room_list li").dblclick(function(e) {
						if ($(this).children("span:last-child").html() == '1') {
							...
							var which = 'check';
							window.parent.parent.JDialog.openWithNoTitle("",base_path +'/page/ipmspage/room_statistics/roomlist_check.jsp?checkid=' + checknumber, 1179, 733);
						} else if ($(this).children("span:last-child").html() == 'W') {
							var roomId = $(this).children("span:first-child").html();
							var logid;
							$.ajax({
								url: base_path + "/getLogIdByRoom.do",
								type: "post",
								dataType: "json",
								data: {roomId: roomId},
								success: function (json) {
									logid = json.data.logId;
									window.JDialog.open("停售房编辑", base_path + "/page/apartment/apartmentmainmenu/apartmentstopselling/stophouse_edit.jsp?logid=" + logid, 882, 512);
								},
								error: function (json) {
							...
							if ($.trim($(this).children("span:nth-child(2)").html()) != null && $.trim($(this).children("span:nth-child(2)").html()).length > 0) {
								var checknumber = $(this).children("span:nth-child(2)").html()
								var which = 'check';
								window.JDialog.openWithNoTitle("",base_path +'/page/ipmspage/room_statistics/roomlist_check.jsp?checkid=' + checknumber, 1179, 733);
							} else {
								showMsg("此房为脏房!");
							}
						}
					...
					$.ajax({
						type : "POST",
						url:base_path + "/roomApartStatusCount.do",
						dataType : "json",
...
function queryarrival(type) {
	$("#houseName").val("");
	$.ajax({
		url:base_path + "/queryApartmentArrival.do",
		type:"post",
		dataType:"json",
		data:{type: type},
		success:function(json) {
...
function queryleavelive(status){
	...
	$.ajax({
		url:base_path + "/queryApartLeavelive.do",
		type:"post",
		dataType:"json",
		...
		success:function(json) {
			if(json.length > 0) {
				...
				$(".room_list li").dblclick(function() {
						var checknumber = $(this).children("span:nth-child(2)").html()
						var which = 'check';
						window.JDialog.openWithNoTitle("",base_path +'/page/apartment/apartmentmainmenu/apartmentroomstatus/roomlist_check.jsp?checkid=' + checknumber + '&type=' + which, 1179, 733);
				});
			} else {
				$(".room_list").html(" ");
			}  
...
function queryroom(status) {
	...
	$.ajax({
		url:base_path + "/queryApartRoomByStatus.do",
		type:"post",
		dataType:"json",
		data: { status: status, roomtype:roomtype, roomfloor:roomfloor },
		success:function(json) {
			if(json.length > 0) {
				...
				$(".room_list li").dblclick(function(e) {
					if ($(this).children("span:last-child").html() == '1') {
						var roomId = $(this).children("span:first-child").html();
						var roomType = $(this).children("span:nth-child(2)").html();
						JDialog.openWithNoTitle("", base_path + "/checkInApartByOrder.do?roomId=" + roomId + "&roomType=" + roomType,1179,780);
					} else if ($(this).children("span:last-child").html() == 'Z') {
						if ($.trim($(this).children("span:nth-child(2)").html()) != null && $.trim($(this).children("span:nth-child(2)").html()).length > 0) {
							var checknumber = $(this).children("span:nth-child(2)").html()
							var which = 'check';
							window.JDialog.openWithNoTitle("",base_path +'/page/apartment/apartmentmainmenu/apartmentroomstatus/roomlist_check.jsp?checkid=' + checknumber, 1179, 733);
						} else {
							showMsg("此房为脏房!");
						}
					} else if ($(this).children("span:last-child").html() == 'T') {
						...
						var logid;
						$.ajax({
							url: base_path + "/getApartLogIdByRoom.do",
							type: "post",
							dataType: "json",
							data: {roomId: roomId},
							success: function (json) {
...
$("#dropse_roomtype").change(function(){
	...
	$.ajax({
		url:base_path + "/queryApartByRoomType.do",
		type:"post",
		dataType:"json",
...
/*	if (label != "空净房" || label != "脏房" || label != "停售房" || label != "所有" || label != "维修房") {
	...
	$.ajax({
		url:base_path + "/queryApartByRoomFloor.do",
		type:"post",
		dataType:"json",
...
if($("#roomId").focus){
	$("#roomId").keydown(function(e){
         ...
         if (key == 13) {
			...
			$.ajax({
				url:base_path + "/queryApartByRoomId.do",
				type:"post",
				dataType:"json",
...
function setroom(roomstatus, type) {
	...
	if (type == 2) {
		if (roomstatus == 'T') {
			//roomId = $(".roomIds").val();
			window.JDialog.open("增加停售房", base_path + "/page/apartment/apartmentmainmenu/apartmentstopselling/stophouse_add.jsp?roomId=" + roomId + "&flag=1",832,462);
			return false;
		} else if (roomstatus == 'W') {
			var roomType  = $("#myRoomType").val();
			window.JDialog.open("添加维修记录",base_path+"/page/apartment/apartmentLeftmenu/apartmentrepair/addRepairApplication.jsp?roomId=" + roomId + "&roomType=" + roomType,605,350);
			return false;
		}
	} else if (type == 3) {
		if (roomstatus == 'T') {
			//roomId = $(".roomIds").val();
			window.JDialog.open("增加停售房", base_path + "/page/ipmspage/stophouse/stophouse_add.jsp?roomId=" + roomId + "&flag=1",832,462);
			return false;
		} else if (roomstatus == 'W') {
	...
	$.ajax({
		url: base_path + "/setApartRoomStatus.do",
		type: "post",
		dataType: "json",
...
function turnto(which) {
	...
	if (which) {
		window.JDialog.openWithNoTitle("",base_path +'/page/ipmspage/room_statistics/roomlist_check.jsp?checkid=' + checknumber + '&type=' + which, 1179, 733);
	} 
}
...
function getRoomType() {
	$.ajax({
        url: base_path + "/loadApartsearchroomtype.do",
		 type: "post",
		 data : {},
		 success: function(json) {
...
function getRoomFloor() {
	$.ajax({
        url: base_path + "/loadApartRoomFloor.do",
		 type: "post",
		 data : {},
		 success: function(json) {
		...
		if (e.which == 3) {
	        ...
	        if ( type == 2) {
		        ...
		        if ($(this).find("span:last-child").html() == 'W') {
		        	...
		        	||  $(this).find("span:last-child").html() == '2') {
		        	if ($.trim($(this).find("span:nth-child(2)").html()) != null && $.trim($(this).find("span:nth-child(2)").html()).length > 0) {
		        		$(".apartliveroom").show();
		        		$(".apartmaintroom").hide();
		        		$(".apartcleanroom").hide();
...
function bindDbclick(type) {
	$(".room_list li").dblclick(function(e) {
		if ($(this).children("span:last-child").html() == '1') {
			if (type == 2) {
				window.parent.JDialog.open("新增合同",base_path+"/page/apartment/apartmentmainmenu/apartmentcontract/addContrart.jsp?roomId"+$(this).children("span:first-child").html(),950,500);
			} 
			...
			if (type == 3) {
				...
				if ($.trim(checknumber) != null && $.trim(checknumber).length > 0) {
					window.parent.parent.JDialog.openWithNoTitle("", base_path + "/page/apartment/apartmentmainmenu/apartmentcontract/apartment_details.jsp?contractid=" + checknumber,1179,640);
				} else {
					window.parent.JDialog.open("维修申请记录详情",base_path+"/repairApartmentData.do?roomId="+$(this).children("span:first-child").html(),950,300);
				}
			} else if (type == 1) {
				if ($.trim(checknumber) != null && $.trim(checknumber).length > 0) {
	...
	/*	else if ($(this).children("span:last-child").html() == '2') {
        	window.JDialog.openWithNoTitle("", base_path + "/page/apartment/apartmentmainmenu/apartmentorder/apartmentOrder_details.jsp?houseorderid=" + $(this).find("span:nth-child(2)").html() + "&status=0",1179,580);	
		} */
	});
}