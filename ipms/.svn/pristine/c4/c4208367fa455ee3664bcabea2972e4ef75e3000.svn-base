...
<!-- 									<th class="header">凭证号</th>
 -->									<th class="header">备注</th>
		...
		<script type="text/javascript" src="<%=request.getContextPath()%>/script/apartment/apartmentmainmenu/apartmentcontract/Zoom.js"></script>
		...
		var status = '<%=request.getParameter("status")%>';
			...
			function consumption(){
				...
				JDialog.open("", path + "/page/apartment/apartmentmainmenu/apartmentcontract/consumption.jsp?strbillid=" + strbillid +"&checkid=" + checkid,680,360);
			}
			...
			function checkout(){
				...
				$.ajax({
					 ...
					 success: function(json) {
					 	if(json.result == 1){
							JDialog.open("退房", path + "/page/apartment/apartmentmainmenu/apartmentcontract/checkout.jsp?checkid=" + checkid, 700, 500);
						}else {
							showMsg(json.message);
						}
					 },
			...
			function ReWriteTd(e){
				...
				$.ajax({//查看该账单对应的合同的状态
			         url: path + "/queryContrartStatus.do",
					 type: "post",
					 data : {billId : billId
					 },
					 success: function(json) {
