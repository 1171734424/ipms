...
<html>
  	...
  	<div style="height:100%;">
		...
		<div class="big_div">
			<div class="left_div">
				...
				<input type="hidden" id="contrart_type" value="${type }"/>
				<ul>
					...
					<li class="next_li">
						<label class="div_label">签订时间</label>
						<input class="div_input text_search" id="contrartStartTime" value="${contrartStartTime }" readonly="readonly" /><!--这是合同签订时间  -->
						<input class="div_input text_search" id="contrartEndTime" value="${contrartEndTime }" readonly="readonly"  style="display:none;"/><!--这是合同交租时间  -->
					</li>
					<li class="next_li">
					...
					<li class="next_li">
						...
						<!--押金差额  -->
						<input id="repayMoneyPartOne"  class="div_input text_search" value="" readonly="readonly" style="display:none;"/>
						<!--房费差额  -->
						<input id="repayMoneyPartTwo"  class="div_input text_search" value="" readonly="readonly" style="display:none;"/>
					</li>
				</ul>
			</div>
			<div class="right_div">
				<ul>
					...
					<li class="next_li">
						<label class="div_label">赔偿</label>
  ...
  <script type="text/javascript">
	...
	function repayBill(){
		...
		var repayMoneyPartOne = $("#repayMoneyPartOne").val();
		var repayMoneyPartTwo = $("#repayMoneyPartTwo").val();
		...
		form.append("contractId", $("#contractId").val());
 		...
 		form.append("repayMoneyPartOne", repayMoneyPartOne);
 		form.append("repayMoneyPartTwo", repayMoneyPartTwo);
		...
		$.ajax({
			 ...
			 success: function(json) {
				 ...
				 if(json.result != '0'){
					 ...
					 if($("#changeHouse").val() != ""){
						 if(!re.test($("#changeHouse").val())){
							 window.parent.showMsg("请输入正数!");
							 return false;
						 }
						 if($("#changeHouse").val() >= 1000000){
							 window.parent.showMsg("不能大于1000000!");
							 return false;
						 }
					 }
  ...
  <script type="text/javascript">
  	$(function(){
  		var kind = '${contrart.status}';
  		var contrartStartTime = '${contrartStartTime }';
  		//有效合同且待住
  		if(kind == '1'){
  			$("#newStartTime").val(contrartStartTime);
  		}else{
  			var now = new Date();
