   	...
   	<div class="header_roomlist">
			...
			<div class="high_header stop fadeIndown">
				...
				<form id="halt_search" method="post">
					<div class="margintop">
						<ul class="clearfix">
							<li><label class="label_name">民宿编号</label><input name="roomid" type="text" value="" id="id" class="text_search"></li>
		...
		<section class="box-content-section fl"  style="width:100%;">
			<section class="box_content_widget fl">
				...
				<iframe name="logFrame" id="logFrame" frameborder="0" width="100%" height="100%" src="showstophouseNew.do?status=1&type=${param.type}"></iframe>
				</div> 
			</section>
		</section>
		</div>
		...
		<script type="text/javascript">
			...
			function getQuerydata(status){
				...
				if(typeof(status)!= "undefined"){
					$("#status").val(status);
					$("#logFrame").attr('src',path + "/showstophouseNew.do?status=" + status + "&type=" + type);				
				}
				else{
					$("#logFrame").attr('src',path + "/showstophouseNew.do?" + $("#halt_search").serialize());	
				}
			}
			...
			$(".createhouse").on("click",function(){
				window.parent.JDialog.open("增加停售房", path + "/page/ipmshouse/stophouse/stophouse_add.jsp",832,400);
