...
<!--	    <script src="<%=request.getContextPath()%>/script/ipms/js/room_urban/jquery-labelauty.js"></script>-->
		...
		function search(){
			...
			$("#applicationIframe").attr("src",base_path+"/querycleanapplycondition.do?cleanstarttime="+cleanstarttime+"&cleanendtime="+cleanendtime
			
			+"&timeArea="+timeArea+"&status="+status+"&applystarttime="+applystarttime+"&applyendtime="+applyendtime);
		}
		
		function addnewapply(){     
		JDialog.open("添加保洁申请",base_path +"/addnewapply.do",690,370);
