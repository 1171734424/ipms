...
<html class="whitebg">
	...
	$("#gdsroomid").bind("click", function() {
		$.ajax({
			url : base_path + "/selectgdsRoomInHouse.do",
			type : "post",
			dataType : "json",
			data : {},
			success : function(json) {
				if (-1 == json.result) {
					...
					JDialog.open("挂房账", base_path + "/gdsroomSelectInHouse.do?totalprice="+$("#totalprice").val(), 600, 300);
				}
			},
		});
	});
   ...
   //数量加按钮
	...
	function cleargdsroom() {
	 ...
	 function gdsworkbill() {
	    ...
	    window.parent.parent.JDialog.open("创建工作账", path + "/page/ipmspage/work_account/creatework_account.jsp?pagetype="+pagetype ,620,289);
	}
	...
	function gdspaysubmit() {
		...
		if ($("#roompay").is(':checked')) {
			if (gdsroomid) {
				$.ajax({
					url : base_path + "/gdsRoompayInHouse.do",
					type : "post",
					dataType : "json",
					data : {
			...
			<script
				src="<%=request.getContextPath()%>/script/ipmsinhouse/leftmenu/goodssale.js"></script>
