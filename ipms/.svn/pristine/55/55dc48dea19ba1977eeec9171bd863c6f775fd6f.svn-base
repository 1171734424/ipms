  ...
  <body>
					...
					<c:forEach items="${result}" var="list">
						<tr ondblclick="showstophaltplan(this)">
							...
							<c:choose>
								...
								<c:otherwise>
									<td><span class="can undonet"  style="background: url(./css/ipms/img/backgroundimg/cry.png) no-repeat center;" ></span></td>
								</c:otherwise>
							</c:choose>
						</tr>
					</c:forEach>
	...
	<script>
		...
		function showstophaltplan(e){
			...
			window.JDialog.open("停售房编辑", path + "/page/ipmspage/stophouse/stophouse_edit.jsp?logid=" + logid + "&startTime=" +startTime + "&endTime=" + endTime, 882,512);
		}
