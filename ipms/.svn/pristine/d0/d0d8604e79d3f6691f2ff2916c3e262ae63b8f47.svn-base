   	...
   	<div class="header_roomlist">
		...
		<section class="box-content-section fl"  style="width:100%;">
			<section class="box_content_widget fl">
				...
				<iframe name="logFrame" id="logFrame" frameborder="0" width="100%" height="100%" src="showstophouse.do?status=1&type=${param.type}"></iframe>
				</div> 
			</section>
		</section>
		</div>
		...
		<script type="text/javascript">
			...
			function getQuerydata(status){
				...
				if(typeof(status)!= "undefined"){
					$("#status").val(status);
					$("#logFrame").attr('src',path + "/showstophouse.do?status=" + status + "&type=" + type);				
				}
				else{
