...
public class GenericDAOImpl extends BaseDAO implements IGenericDAO {
	 ...
	 * @throws DAOException
	 */
	public String findMultiSQL(String sql, boolean... inDB) throws DAOException {
		String[] arrsql = sql.split(","); 
		if (inDB != null && inDB.length > 0 && inDB[0]) {
			String dbType = DataSourceContextHolder.getCustomerType();
			String sqltext = "";
			for (String subsql : arrsql) {
				Object si = findOneByProperties(SqlInfo.class, "sqlName", subsql, new Object[] { "dbType", dbType });
				sqltext = sqltext + ((SqlInfo) si).getSqlInfo() + " ";
			}
			return sqltext;
		}
		return sql;
