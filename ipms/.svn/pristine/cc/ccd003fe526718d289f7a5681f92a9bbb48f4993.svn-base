  	...
  	<div class="excep_margin">
  	   <form id="pagerForm" name="pagerForm" action="" target="_self">
	    	...
	    	<table id="myTable" class="myTable" border="0" width="100">
				...
				<tbody id="info">
					<c:forEach items="${exceptiondata}" var="exceptiondata">
						<tr>
							<td>${exceptiondata["DAILYTIME"]}</td>
							<td>${exceptiondata["EXCEPTIONID"]}</td>
							<td>${exceptiondata["PARAMDESC"]}</td>
							<td>${exceptiondata["RECORDTIME"]}</td>
							<td>${exceptiondata["REMARK"]}</td>
							<td>${exceptiondata["STATUS"]}</td>
