...
<html class="whitebg">
						...
						function() {
							if ($(this).is(':checked')) {
								...
								if ($(this).attr('id') == "roompay") {
									...
									$("#gdsroomid").attr("disabled", true);
									$("#cashpayvalue").show();
									$("#cashpayvalue").val($("#totalprice").text());
									$("#cardpayvalue").val("");
									$("#cardpayvalue").hide();
									//$("#costproject").hide();
									$("#payproject").show();
	...
	$("#gdsroomid").bind("click", function() {
		$.ajax({
			url : base_path + "/selectApartgdsRoom.do",
			type : "post",
			dataType : "json",
			data : {},
			success : function(json) {
				if (-1 == json.result) {
					...
					JDialog.open("挂房账", base_path + "/gdsroomApartSelect.do?totalprice="+$("#totalprice").text(), 600, 300);
				}
			},
		});
	});
   ...
   //数量加按钮
	...
	function gdspaysubmit() {
		...
		if ($("#roompay").is(':checked')) {
			if (gdsroomid) {
				$.ajax({
					url : base_path + "/gdsApartRoompay.do",
					type : "post",
					dataType : "json",
					data : {
			...
			if (gdsworkbill) {
				$.ajax({
					url : base_path + "/gdsApartDumbpay.do",
					type : "post",
					dataType : "json",
					data : {
						'gdscheckid' : gdscheckid,
			...
			if(!gdsworkbill){
			    showMsg("请选择工作账（无工作账请先创建工作账）");
					...
					$.ajax({	url : base_path + "/gdsApartRoompaycash.do",
								type : "post",
								dataType : "json",
								data : {
									'gdscheckid' : gdscheckid,
			...
			if(!gdsworkbill){
			    showMsg("请选择工作账（无工作账请先创建工作账）");
					...
					$.ajax( {
								url : base_path + "/gdsApartRoompaycard.do",
								type : "post",
								dataType : "json",
								data : {
									'gdscheckid' : gdscheckid,
			...
			<script
				src="<%=request.getContextPath()%>/script/apartment/apartmentLeftmenu/apartmentgoods/goodssale.js"></script>
