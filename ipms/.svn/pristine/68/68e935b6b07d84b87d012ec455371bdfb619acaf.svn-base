  ...
  <body>
						...
						<tr ondblclick="showstophaltplan(this)">
							...
							<c:choose>
								<c:when test="${list.STATUS == 1}">
									<td>
										<span class="newcancle" onclick="tipcancel(this)" >取消</span>
									</td>
								</c:when>
								<c:otherwise>
									<td><span class="notcancle">取消</span></td>
								</c:otherwise>
							</c:choose>
						</tr>
					</c:forEach>
	...
	<script>
		...
		function showstophaltplan(e){
			...
			window.JDialog.open("停售房编辑", path + "/page/ipmshotel/hotelStopHouse/stophouse_edit.jsp?logid=" + logid + "&startTime=" +startTime + "&endTime=" + endTime, 882,512);
		}
		...
		function cancelhaltplan(logid){
			$.ajax({
		         url: path + "/hotelCancelHaltplan.do",
				 type: "post",
				 data : {logid : logid},
