...
.contractbutton{
   ...
   background-color: #5A5D9D;
	...
	.pdepartaddPic:hover,.pdepartsubmit:hover,.pdepartcancle:hover,.pdepartadd:hover{
		font-weight:bold;
	}
	.pdepartcancle{
	float:left;
...
display:none;
  ...
  <body onload="init()">
  <div class="topdiv">
   <input type="hidden" name="houseid" id="houseid" value="" class=""  /> 
    <table class="branch_table">
    	<tr>
    	<td class="must">民宿名称</td>
    	<td><input class="inputStyle" id="housename" maxlength="15" placeholder="请输入1-15个有效字符"  value="" /></td>
    	<td class="must">门牌号</td>
    	<td><input class="inputStyle" id="houseNo" maxlength="6" placeholder="请输入1-6个数字" value="" />
    	</td>
    	<td class="must">楼层</td>
...
<!--			onkeyup="var reg = /^[\da-z]+$/i; if(!reg.test(this.value)) this.value = ''" name="roomType"></input></td>-->
		...
		<td >
			<input class="inputStyle" id="roomName" maxlength="8" placeHolder="单室套/3室1厅"	name="roomName"></input>
		</td>
		
		<td class="must">装修风格</td>
    	<td><input class="inputStyle" id="decStyle" value="" /></td>
    	<td class="must">小区名称</td>
    	<td><input class="inputStyle" id="communityName" value="" /></td>
    	<%-- <td id="no_input" class="must">场景</td>
		<td >
			<select id="theme" class="inputStyle" name="theme" disabled >
				<c:forEach items="${themeList}" var="themeVar">
				<c:if test="${themeVar.CONTENT eq '3'}"> 
...
<!--     	<input type="hidden" id="staffid" name="staffid" value="" /> -->
		...
		<td class="must">床位</td>
    	<td><input class="inputStyle" id="beds" maxlength="2" value="" onkeyup  ="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'');}else{this.value=this.value.replace(/\D/g,'');}",
   onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"/></td>
    	...
    	<c:forEach items="${beddescList }" var="beddescList">
			<option value="${beddescList.paramDesc}" id="" >${beddescList.content}</option>
			</c:forEach>
		</select></td>
    	...
    	<c:forEach items="${positionList}" var="positionList">
			<option value="${positionList.paramDesc}" id="" >${positionList.content}</option>
			</c:forEach>
			</select>
		</td>
		<td class="must">详细地址</td>
    	<td><input class="inputStyle" id="preLocation" value="" maxlength="27"  /><!-- &nbsp—&nbsp -->
    	<!-- <input class="inputStyle" id="aftLocation" value="" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'');}else{this.value=this.value.replace(/\D/g,'');}",
	  ...
	  <td class="must">基准价</td>
    	<td><input class="inputStyle2" id="originalprice" value="" maxlength="8"/><span>元</span></td>
    	<td class="must">经度</td>
    	<td><input class="inputStyle" id="longitude" value="" placeholder="例如：160.12365"/></td>
    	<td class="must">纬度</td>
    	<td><input class="inputStyle" id="latitude" value="" placeholder="例如：89.12365"/></td>
	  </tr>
	  <tr>
	  <td class="must">保洁费</td>
    	<td><input class="inputStyle2" id="cleanprice" value="" maxlength="8"/><span>元</span></td>
    	<td class="must">押金</td>
    	<td><input class="inputStyle" id="cashpledge" value="" />元</td>
<td class="must">宽带</td>
    	<td>
	  ...
	  <td class="must">建筑面积</td>
    	<td><input class="inputStyle2" id="area" value="" maxlength="4" onkeyup  ="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'');}else{this.value=this.value.replace(/\D/g,'');}",
    	...
    	<td class="must" style = "display:none">状态</td>
    	...
    	<select class="inputStyle" id="status"  style="display:none;">
            <option value="1">空</option><!-- 默认空房 不显示出来 -->
		</select>
		</td>
         ...
         onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" value="" maxlength="8" placeholder="请输入1~8位正整数"/></td>
    	...
    	<td class="must district_add">区</td>
    	<td><select class="inputStyle district_add" id="district" onchange="chooseDistrict()">
		...
		<td class="circle_add">商业圈</td>
		<td>
		<select class="inputStyle circle_add" id="circle" onchick="chooseCircle()">
    	...
    	<td class="school_add">学校</td>
    	<td><select class="inputStyle school_add" id="school" onclick="chooseSchool()">
			 <c:if test="${fn:length(districtList) eq 0}">
			<option value="">暂无学校记录</option>
			</c:if> 
			<c:forEach items="${districtList }" var="schoolList">
			<option value='${districtList["ADMINI_CODE"]}' id="schoolopt" <c:if test="${districtList['ADMINI_CODE'] eq district.adminiCode}">selected="selected"</c:if> >${districtList["ADMINI_NAME"]}</option>
			</c:forEach>
			</select></td>
		<td class="scenic_add">景点</td>
    	<td>
    	<select class="inputStyle scenic_add" id="scenic" onclick="chooseScenic()">
		 <c:if test='${(fn:length(streetList) eq 0) or street.adminiCode eq ""}'>
		<option value="">暂无景点记录</option>
		</c:if> 
		<c:forEach items="${streetList }" var="scenicList">
		<option value='${streetList["ADMINI_CODE"]}' id="scenicopt" <c:if test="${streetList['ADMINI_CODE'] eq street.adminiCode}">selected="selected"</c:if>>${streetList["ADMINI_NAME"]}</option>
		</c:forEach>
		</select>
		</td>
		
		<td class="subway_add">地铁站</td>
		<td>
		<select class="inputStyle subway_add" id="subway" onchick="chooseSubway()">
			 <c:if test="${fn:length(circletList) eq 0}">
				<option value="">暂无地铁记录</option>
				</c:if> 
				<c:forEach items="${circletList }" var="subwwayList">
				<option value='${circletList["ADMINI_CODE"]}' id="subwayopt" <c:if test="${circletList['ADMINI_CODE'] eq circle.adminiCode}">selected="selected"</c:if>>${circletList["ADMINI_NAME"]}</option>
				</c:forEach>
			</select></td>
 ...
 --%>
		...
		<tr>  
		<td >房间设施</td>
		<td colspan="5"><input id="roomDevice" onclick="dev()" class="roomDevice" name="device" readonly></input></td>
		</tr>
		<tr> 
		 <td>入住须知</td>
		 <td colspan="5"><input id="roomTip"  onclick="tips()" class="roomDevice" value="" name="tip" readonly></input></td>
		</tr>
		<tr> 
		 <td>特色服务</td>
		 <td colspan="5"><input id="roomService"  onclick="services()" class="roomDevice" value="" name="roomService" readonly></input></td>
		</tr>
		<tr>
		<td class="must">关键字</td><td colspan="5"><input class="keyword_input" id="keyword" placeholder="多个关键字以逗号分割，不能超过6个关键字，每个关键字字数不能超过5个字符！"/></td>
		</tr>
...
<!--    	<tr><td>备注</td><td colspan="3"><input class="inputStyle" id="remarkInput" value=""/></td></tr>-->
		...
		<tr>
			...
			<td colspan="5">
				<textarea  style="width : 921px;height: 80px; font: 13.3333px Arial;" id="housedesc" name="housedesc" class="inputStyle"   maxlength="1000" placeholder="请输入1000字以内描述！"></textarea>
			</td>
		</tr>
		<tr>
        <td>备注</td><td colspan="5"><textarea  style="width : 921px;height: 80px; font: 13.3333px Arial;" id="remarkInput" name="remarkInput" class="inputStyle"  maxlength="100"  placeholder="备注内容格式：距离xx(景点)xx(米/公里)"></textarea></td>
		</tr>
    </table>
    <div class="contractbutton">

     <%--<div class="pdepartaddPic" onclick="toAddHousePic()">新增图片</div>
...
<!--标签div-->
	...
	<div class="picContent" style="display:none">
	<div class="innerAdderHousePic" >
		<form id="basicInfo" name="basicInfo" class="basicInfo"  method="POST"
							enctype="multipart/form-data">
			  <table>
			     	<tr><td  class="tdone">门店头图</td><td class="tdtwo"><input   id="branchHeadPic" name="branchHeadPic" type="file" placeholder="请选择图片！" name="roomTypePic"></input></td></tr>
			   		<tr><td  class="tdone">门店内景</td><td class="tdtwo"><input id="div_imgfile" type="button" value="选择图片"></input></td></tr>
			   <!--图片预览容器-->
				<tr>
					<input type="hidden" name="branchId" value="" id="branchId"/> 
				<td  colspan="2">
				<div id="div_imglook" style = "width:100%">
					<div style="clear: both;" id="picBox"></div>
				</div>
				</td>
				</tr>
			 </table>
		<div class="contractbutton picButton">
		 <div class="pdepartsubmit" onclick="submitPic()">确定</div>
		  <div class="pdepartcancle" onclick="hiddenAddPic()">取消</div> 
		 </div>
		</form>
	</div>
	</div>
...
/* getLatlng(); */
      ...
      function getLatlng(){
    	  ...
    	  var optionsCityText = $("#cityCode").val();
	  ...
	  $.ajax({
			...
			if(json.result == 0){
			    showMsg(json.message);
			        ...
			        $("#branchId").val(json.message);
			       // submitRoomType(json.message);
...
/*   function chooseCity(){
  ...
  function showMsg(msg, fn) {
   ...
   function toAddHousePic(){
	   $(".picContent").show();
   }
   function submitPic(){
	   $(".picContent").hide();
   }
   function hiddenAddPic(){
	   $("#branchHeadPic").val("");
	   $(".imgfile").remove();//移除图片file
	   $(".lookimg_delBtn").parent().remove();//移除图片显示
	  /*  var file = $(":file");  
