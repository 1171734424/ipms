	...
	<body>
		...
		<div class="customer_div">
			...
			<div class="check_four">
				<span class="button_margin" onclick="preroomout()">退房</span>
				<!-- <span class="button_margin button_color" onclick="updateCustomer()">确定</span> -->
				<span class="button_margin" onclick="showcheck()" style="display: none;" >打印</span>
		...
		<script src="<%=request.getContextPath()%>/script/ipms/js/workbillroom/tipInfo-lj.js"></script>
		...
		<script >
			...
			function roomout(e) {
				...
				window.parent.JDialog.open("退房备注", path + "/page/ipmshotel/room_statistics/checkoutRemark.jsp?checkId=" + checkid , 450,200);
			}
			/*function roomout(e) {
				if(e == "cancel"){
					return false;
			...
			function preroomout(){
				...
				$.ajax({
					 ...
					 success: function(json) {
					 	if(json.result == 1){
					 		if(json.data == 1){
					 			showMsg("存在关联房,是否确认操作?", 'roomout("confirm")', 'roomout("cancel")');
					 		}else if(json.data == 0){
					 			roomout("confirm");
					 		}
					 	}
					 ...
					 success: function(json) {
					 	if(json.result == 0){
							...
							$(window.parent.document).find("#checkid").val(json.data.checkId);
					 		window.parent.document.getElementById("customer").innerHTML='<iframe src="' + path +'/page/ipmshotel/room_statistics/customer.jsp?checkid=' + json.data.checkId + '" width="100%" height="100%" frameborder=no  scrolling="no"></iframe>';
					 	}
					 },
					 error: function(json) {}
				});
			...
			function registermember(){
				if(customerCheck)
					return false;
				window.parent.JDialog.open("会员注册", path + "/page/ipmshotel/room_statistics/mebregister.jsp" , 500,220);
			}
