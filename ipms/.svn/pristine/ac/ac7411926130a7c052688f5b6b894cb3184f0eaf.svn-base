  ...
  <body>
   	<div class="detail_wrap_margin detail_color">
			...
			$.ajax({
				 ...
				 success: function(json) {
				 	if (json.length <= 0) {
						...
						window.JDialog.open("选房", path + "/page/ipmshotel/order/showRoom.jsp?orderId=" + orderid , 1000, 650);
					}
				 },
				 error: function(json) {}
			});
		...
		function registermember(){
			...
			JDialog.open("会员注册", path + "/page/ipmshotel/leftmenu/leftorder/ordermemberadd.jsp" , 800, 410);
		}
		//订单查看
		function checkOrder(){
			var customerid;
			...
			window.parent.JDialog.openWithNoTitle("", path + "/page/ipmshotel/order/order_check.jsp?orderId=" + $("#orderid").val() + 
		...
		function showeditorder() {
			...
			window.JDialog.open("修改订单", path + "/page/ipmshotel/order/editorder.jsp?orderId=" + $("#orderid").val() , 700, 400);
		}	
		...
		function cancleorder() {
			...
			window.JDialog.open("取消订单", path + "/page/ipmshotel/order/secondquery.jsp?orderId=" + $("#orderid").val() , 200, 100);
		}
