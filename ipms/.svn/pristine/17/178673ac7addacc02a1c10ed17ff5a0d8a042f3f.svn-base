		...
		<form id="beasform" name="beasform" class="beasform" method="POST" enctype="multipart/form-data">
			<div class="ul_div">
				<div>
					...
					<input class="first_divM" name="brandtype" id = "brandtype" onclick="toChooseHouse()" value="${comment.HOUSENAME }"  placeholder="民宿选择">
					<input id="brandtype_CUSTOM_VALUE" name="brandtype_CUSTOM_VALUE" value="${comment.BRANCH_ID }" type="hidden"/>
					<label>图片</label>
				...
				<div>
					...
					<input name="serviceScore" id = "serviceScore" value="${ comment.SERVICE_SCORE}" maxlength="3" onkeyup="value=value.replace(/^[^0,1,2,3,4,5]{1}$/g,'')" placeholder="请输入0-5的数字" />
					<label>卫生评论</label>
					<input name="serviceComment" id = "serviceComment" value="<c:if test='${ comment.SERVICE_COMMENT =="null"}'></c:if><c:if test='${ comment.SERVICE_COMMENT !="null"}'>${ comment.SERVICE_COMMENT}</c:if>" maxlength="100" placeholder="卫生评论" />
				</div>
				<div style="margin-top: 15px; margin-bottom: 18px;">
					<label style="color:red">设施评分</label>
					<input name="facilityScore" id = "facilityScore" value="${ comment.FACILITY_SCORE}" maxlength="3" onkeyup="value=value.replace(/^[^0,1,2,3,4,5]{1}$/g,'')" placeholder="请输入0-5的数字"/>
					<label>设施评论</label>
					<input name="facilityComment" id = "facilityComment" value="<c:if test='${ comment.FACILITY_COMMENT =="null"}'></c:if><c:if test='${ comment.FACILITY_COMMENT !="null"}'>${ comment.FACILITY_COMMENT}</c:if>" maxlength="100" placeholder="设备评论" />
				</div>
				<div style="margin-top: 18px; margin-bottom: 18px;">
					<label style="color:red">安全感评分</label>
					<input class="securityInput" name="securityScore" id = "securityScore" value="${ comment.SECURITY_SCORE}" maxlength="3" onkeyup="value=value.replace(/^[^0,1,2,3,4,5]{1}$/g,'')" placeholder="请输入0-5的数字" />
					<label>安全感评论</label>
					<input class="securityInput" name="securityComment" id = "securityComment" value="<c:if test='${ comment.SECURITY_COMMENT =="null"}'></c:if><c:if test='${ comment.SECURITY_COMMENT !="null"}'>${ comment.SECURITY_COMMENT}</c:if>" maxlength="100" placeholder="安全感评论" />
				</div>
				<div>
					<label style="color:red">评论人</label>
					<input name="recorduser" id = "recorduser"  style="margin-left: 32px;"  placeholder="评论人" value="${comment.RECORD_USER}"/>
				</div>
				<div style="margin-top: 18px">
					<label>评论描述</label>
					<textarea maxlength="100" name="commentContent" id = "commentContent" value="${commentContent }"  placeholder="评论描述有效字符在100以内"></textarea>
				</div>
				<%-- <div>
					<label>快捷标签</label>
					<input name="shortcutName" id = "shortcutName" onclick="toShortcutName()" value="${specialLabel }" style="margin-left: 17px;"  placeholder="快捷标签选择"/>
					<input id="shortcutName_CUSTOM_VALUE" name="shortcutName_CUSTOM_VALUE" value="${comment.SPECIAL_LABEL }" type="hidden"/>
				</div> --%>
			</div>
		</form>
			<div class="button_div" >
				<button class="left_button" onclick="window.parent.JDialog.close()" >关闭</button>
				<button class="right_button" onclick="addComment()">修改</button>
			</div>
		</div>
