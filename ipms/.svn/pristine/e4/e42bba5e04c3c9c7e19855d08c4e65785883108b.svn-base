   	...
   	<%--  <div class="detail_wrap_margin detail_color">
			...
			<div class="order_margin house_margin">
				<section class="detail_one">
					...
					<ul class="clearfix">
						<li><label class="label_name">民宿编号</label><input name="民宿编号" type="text" id="houseid"  class="check room_select" disabled></li>
		...
		<script>
			...
			function overstay(){
				window.JDialog.open("续住", path + "/page/ipmshouse/housecheck/overstay.jsp?leavetime=" + $("#leavetime").val() + "&houseorderid=" + houseorderid, 700, 350);
			}
			...
			function checkOut() {
				window.JDialog.open("民宿退房", path + "/page/ipmshouse/housecheck/secondquery.jsp?orderId=" + houseorderid , 400,250);
				/* $.ajax({
		...
		function lookHouseOrder(){                            
			window.parent.JDialog.openWithNoTitle("", path + "/page/ipmshouse/houseorder/houseOrder_check.jsp?houseorderid=" + houseorderid , 1179, 733);
		}
	</script>
  </body>
</html>
