     ...
     <div id="multiplesale" class="multiplesale">
		...
		//根据勾选的商品计算总价和数量
         ...
         $("#paycheckbox").children("label").find('input[type=checkbox]').click(function(){
            ...
            $("#gdsroomid").bind("click",function(){     
           		 //JDialog.open("挂房账", base_path + "/gdsroomSelect.do?totalprice="+$("#totalprice").val(), 600, 300);
         		$.ajax({
        			...
        			success : function(json) {
        				if (-1 == json.result) {
        					...
        					JDialog.open("挂房账", base_path + "/gdsroomApartSelect.do?totalprice="+$("#totalprice").val(), 600, 300);
        				}
        			},
        		});
            });
		...
		if ($("#roompay").is(':checked')) {
           ...
           if(!gdsworkbill){
            ...
            $.ajax({
					url : base_path + "/gdsApartRoompaycash.do",
					type : "post",
					dataType : "json",
					data : {
						'gdscheckid' : gdscheckid,
