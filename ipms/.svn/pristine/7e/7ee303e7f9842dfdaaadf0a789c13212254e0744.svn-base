...
// 				}
			...
			function checkOut() {
				/* $.ajax({
					 ...
					 success: function(json) {
						 if(json["amount"] == 0){ */
							 window.JDialog.open("公寓退房", path + "/page/ipmspage/apartmentcheckin/secondquery.jsp?orderId=" + contractid , 400,250);
						 /* } else {
							 showMsg("请结清水电费!");
						 }
					 },
			...
			function queryBill(num){
				window.JDialog.open("账单", path + "/page/ipmspage/leftmenu/apartmentrefund/apartment_bill.jsp?checkid=" + contractid+"&status="+num , 1000, 500);
			}
