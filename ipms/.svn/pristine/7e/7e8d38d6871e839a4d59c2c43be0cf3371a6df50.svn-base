	...
	<body>
		<div class="room_margin">
	 	<div class="header_search">
	 		<form action="" method="">
	  		<label>营业日期</label>
	  		<input id="mdate" type="text" name="mdate" class="date_txt" disabled/>
	 			<span class="forward">远期房态统计</span>
	 		</form>
	 	</div>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				laydate({
					...
					choose: function(){
						...
						$("#currentFrame").attr("src","currentHouse.do?madate="+$("#mdate").val());
				        }  
				});
				var now = new Date();
				...
				$("#currentFrame").attr("src", "currentHouse.do?madate=" + time);
			})
			...
			$(".forward").on("click",function() {
				JDialog.openWithNoTitle("", base_path + "/page/ipmshouse/topMenu/houseStatistics/forwardStatus.jsp", 1179,738);
			});
		</script>
	</body>
</html>
